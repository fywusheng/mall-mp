<template>
  <view class="ordermanage">
    <!-- 头部导航栏 -->
    <tabs v-model="currentIndex" height="114rpx" lineColor="#FF711A" :fixed="false" fontSize="36rpx"
      color="#999" activeColor="#333" :tabs="items" :scroll="true" field='name' @change="clickItem">
    </tabs>
    <!-- <navigation-bar :alpha="0" statusBackGround="#ff5d28" :style="{height: navHeight + 'px'}">
      <view slot="title1">
        <view class="navigation-bar flex-h flex-c-s">
          <image class="image" src="https://ggllstatic.hpgjzlinfo.com/static/home/home-logo.png"
            mode="scaleToFill" />
        ...
        <view class="tabCur">
          <tabs v-model="currentIndex" height="100rpx" lineColor="#FF711A" :fixed="false"
            fontSize="36rpx" color="#999" activeColor="#333" :tabs="items" :scroll="true"
            field='name' @change="clickItem"></tabs>
        </view>
      </view>
    </navigation-bar> -->
    ...
    <view class="cont" v-if="loading == 2">
      <view class="collectCenter">
        <hr-pull-load @refresh='refresh' @loadMore='loadMore' height='-2' :pullHeight='50'
          :maxHeight='100' :lowerThreshold='30' :newLoading='newLoading' :bottomTips='bottomTips'
          :isAllowPull="false" :isTab='true' ref='hrPullLoad'>
          <!-- 全部订单 -->
          ...
          <!-- 酒店订单 -->
          <hotel-order v-if="currentIndex === 5" :list="tablist[currentIndex]"
            @resetOptions="resetOptions" />

          <!-- 保险订单 -->
          <insurance-order v-if="orderSource === '3'" :list="tablist[currentIndex]"
            @resetOptions="resetOptions" />

          <!-- 商城订单 -->
          <shop-order v-if="orderSource === '9'" :list="tablist[currentIndex]"
            @resetOptions="resetOptions" />
        </hr-pull-load>
      </view>
    </view>
