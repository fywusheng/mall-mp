  ...
  <view class="pages">
    <view class="nearby-list" v-if="list.length > 0">
      ...
      <template v-for="(item,index) in list">
        <!-- 附近优惠 -->
        <view v-if="![4,9,13].includes(item.orderSource)" class="item"
          @click="handleOrderInfo(item)" :key="index">
        ...
        <!-- 酒店 -->
        <view v-if="item.orderSource === 4" class="hotel-item" :key="index"
        ...
        <view class="insurance-item" v-if="item.orderSource === 13" :key="index">
          <insurance-order-item :item="item" :key="item.id">
          </insurance-order-item>
        </view>

        <!-- 商城 -->
        <shop-order-item v-if="item.orderSource === 9" :item="item.orderDTO" :key="item.orderId">
        </shop-order-item>
