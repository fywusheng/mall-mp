...
<template>
  <view class="error-tip">
    ...
    <navigation-bar :alpha="1">
      <view slot="title1">
        <view class="navigation-bar flex-h flex-c-s" :style="{height: '44px'}">
          ...
          <text
            class="navigation-bar__title fs-44 c-black flex-1">{{resultType===0?'支付成功':'支付失败'}}</text>
        </view>
      </view>
    </navigation-bar>
    <!-- #endif -->
    ...
    <view class="page-header mt-16">
      ...
      <view class="error-txt">{{resultType===0?'支付成功':'支付失败'}}</view>
      <view class="pay-account">¥{{payAmount | formaterMoney}}</view>
    </view>
    <view class="page-content">
      ...
      <view class="item">
        ...
        <view class="cont">{{payWay[formData.payment]?payWay[formData.payment]:'无'}}</view>
      </view>
    </view>
    ...
    <!-- 订单关闭 -->
    <modal ref="payInfor" title="支付超时" confirmText='确定' cancelText=' ' @confirm='handleConfirm'>
      <view slot="text">
  ...
  methods: {
    ...
    handleOrderDetail() {
      ...
      switch (this.formData.orderSource) {
        ...
        case 9:
          uni.reLaunch({
            url: '/sub-pages/me/order-detail/main?id=' + this.formData.transactionSerialNo + '&popUpType=2'
          })
          break
        default:
          uni.reLaunch({
    ...
    handleRePay() {
      ...
      if (this.formData.orderSource === 0) {
        ...
        if (mine > 30) {
          ...
          const url = 'https://ggll.hpgjzlinfo.com/#/checkstand?cashId=' + this.formData.cashId
          uni.redirectTo({
            url: `/pages/common/webpage?url=${encodeURIComponent(url)}`
          })
        }
    ...
    async checkNeedPopUp() {
      ...
      if (code === '200') {
        if (data.needPopUp === 1) { this.$refs.pointPop.open(data) }
      } else {
        console.log('失败')
      }
    }
