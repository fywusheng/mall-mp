  ...
  <view class="open-online-pay">
    ...
    <view class="page-header mt-16">
      <view class="img-warpper">
        <image class="icon-circle" :src="icon.auth" />信息加密处理,仅用于银行验证,请验证您本人的银行卡,认证通过后身份信息不可更改
      </view>
      <view class="bank-card" :style="{ background: cardInfo.cardColor }">
        <image class="icon-bg" src="https://ggllstatic.hpgjzlinfo.com/static/pay/icon-bank-pattern.png" mode="scaleToFill" />
      ...
      <view class="item">
        ...
        <view class="cont">
          <input 
            ...
            @focus="handleFocus"
            type="number">
          <image v-if="cardInfo.phone.length>0" class="icon-delete" :src="icon.delete" @click="clearTel"/>
          <image class="icon-circle-gre" :src="icon.circleGre" @click="handlePopPhoneModal"/>
        </view>
      </view>
    </view>
    ...
    <view class="xieyi">
        ...
        <view class="text"> 我已阅读并同意 <text class="blue" @click="handleUserAgreementClick">《国家老龄服务平台实名认证协议》</text> </view>
    </view>

    <view class="page-footer">
      <button class="btn btn-warning" :style="{opacity: enableNext?1:0.5}" :disabled="!enableNext" @click="handleNext">下一步</button>
    </view>
...
export default {
  ...
  methods: {
    ...
    handleNext(){
      ...
      let params = { ...this.cardInfo }
      if(!this.editFlag){
        params.phone = this.firstPhone
      }
      uni.navigateTo({
         url: `/pages/pay/verificat-code?cardInfo=${encodeURIComponent(JSON.stringify(params))}`
      });
    },
    // 协议按钮
    handleCheckXieyi(){
