<template>
  <view class="add-bank-card">
    ...
    <view class="_cont" v-if="isRealName === '1'">
      ...
      <view class="page-header mt-16">
        <view class="input-item">
          <input v-model="preCardNo" maxlength="23" class="input-card" type="text" :focus="focus"
            :adjust-position="false" placeholder-class="placeholder" placeholder="点击输入本人的银行卡号"
            @input="handleChange">
        ...
        <view class="input-txt">请添加您本人的银行卡，享受更便捷的银联服务</view>
        <button class="btn-next" :style="{opacity: enabledNext? 1: 0.5}" @click="handleNext"
          :disabled="!enabledNext">下一步</button>
        <!-- <view class="middle">
    ...
    <real-name-pop ref="realpop" @succFlag="succFlag" :closeGoUrl="closeUrl"
      :maskClick="maskClick" />
  ...
  data() {
    return {
      ...
      icon: {
        ...
        bankArrow: 'https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-arrow-rightward-grey.png',
  ...
  methods: {
    succFlag(flag) {
      if (flag == 1) {
        ...
        this.isRealName = '1'
        // 去实名成功结果页面
        uni.redirectTo({ url: `/pages/user-center/real-name-result2?back=${"/pages/pay/add-bank-card?isRealName='1'"}` })
      }
    },
    ...
    handlePhone() {
      ...
      uni.chooseImage({
        ...
        success: (res) => {
          ...
          getLessLimitSizeImage('canvas', file, 0.1, 750, (imagePath) => {
    ...
    getBankInfoByImg(image64) {
      ...
      api.getBankCardInfoByImage({
        ...
        success: (resinfo) => {
          ...
          uni.navigateTo({
            url: `/pages/pay/confirm-card-no?cardInfo=${encodeURIComponent(JSON.stringify(resinfo))}`
          })
        },
        fail: (res) => {
          uni.hideLoading()
