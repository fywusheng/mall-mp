  ...
  <view class="select-card-type">
    ...
    <view class="page-header mt-16">
      <view class="bank-card">
        <image class="icon-bg" :src="icon.bg" />
        <image class="icon-back" :src="icon.delete"  />
        <view class="bank-name">中国银行储蓄卡</view>
      </view>
      
      <view class="title">请选择您要绑定的卡类型</view>
      <view class="check-group">
        <template v-for="item in checkList">
          <view :key="item.label" class="check-item" :class="{active: item.checked}" @click="handleRaioChecked(item)">
            <image  :class="item.checked?'icon-check':'icon-noCheck'" :src="item.checked?icon.checked:icon.noChecked" />
  ...
  data() {
    return {
      ...
      icon: {
        ...
        bg: 'https://ggllstatic.hpgjzlinfo.com/static/pay/icon-bank-bg.png'
      },
      // 导航栏高度
      //#ifdef MP-WEIXIN
