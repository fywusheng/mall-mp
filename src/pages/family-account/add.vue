<template>
  <view class="add">
    ...
    <!-- #endif -->
    <!-- #ifdef MP-WEIXIN -->
    <navigation-bar :title="'添加亲情账号'" title-color="#ffffff" back-button-style="white"
      :shows-back-button="true" />
    <!-- #endif -->
    <image class="background" mode="scaleToFill"
    ...
    <view class="card bg-white m-0-32 br-24">
      ...
      <template v-if="hasPhoneNumber">
        ...
        <template v-if="canBind">
          ...
          <view class="account flex-h mt-16 br-8" v-for="(item, index) in list1" :key="index">
            <image class="state-icon" @click="changeList(1,index)"
              :src="item.checked === true? checked: unChecked" />
            <view class="flex-v ml-55">
              <text class="name fs-44 fw-500 c-black mb-32">{{item.supportName}}</text>
              ...
              <view class="row flex-h">
                ...
                <text class="fs-36 c-black">{{item.idCard | idCardNumberFilter}}</text>
              </view>
              <view class="row flex-h">
                <text class="fs-36 c-lightgrey">联系电话：</text>
                <text class="fs-36 c-black">{{item.phone | phoneFilter}}</text>
              </view>
            </view>
            <image class="stamp" mode="scaleToFill"
              src="https://ggllstatic.hpgjzlinfo.com/static/family-account/icon-support-stamp-1.png" />
          </view>
          <view class="add-view p-32-0 flex-h flex-c-c" v-if="list1.length === 0">
            <button class="add-button fs-36 c-primary" @click="jumpSupport" hover-class="none">
              +添加赡养人
            </button>
          </view>
          <text class="fs-40 fw-400 c-black mt-36">扶养人</text>
          <view class="account flex-h mt-16 br-8" v-for="(item, index) in list2" :key="index">
            <image class="state-icon" mode="scaleToFill" @click="changeList(2,index)"
              :src="item.checked === true? checked: unChecked" />
          ...
          <view class="add-view p-32-0 flex-h flex-c-c" v-if="list2.length === 0">
            <button class="add-button fs-36 c-primary" @click="jumpSupport" hover-class="none">
              +添加扶养人
            </button>
          </view>
        ...
        <view class="flex-v flex-c-c m-80-0" v-else>
          <text class="tips fs-36 c-black">
            您还没有添加赡养扶养关系，添加后才可绑定亲友
          </text>
          <button class="add-button fs-36 c-primary mt-32" @click="jumpSupport" hover-class="none">
            添加赡养扶养关系
          </button>
        </view>
      </template>
      <template v-else>
        ...
        <view class="row unphone flex-h flex-c-s">
          ...
          <input class="flex-1 fs-40 c-black" placeholder="请输入姓名" placeholder-class="placeholder"
            v-model="params.name" />
        </view>
        <view class="row unphone flex-h flex-c-s">
          <text class="title fs-40 fw-400 c-black">身份证号</text>
          <input class="flex-1 fs-40 c-black" placeholder="请输入身份证号" placeholder-class="placeholder"
            type="idcard" maxlength="18" v-model="params.idCardNumber" />
        </view>
        <view class="row unphone flex-h flex-c-s">
          <text class="title fs-40 fw-400 c-black">选择关系</text>
          <picker class="flex-1" :range="relations" :value="tenantIndex" :range-key="'relationName'"
            @change="handleRelationChange">
            <text class="fs-40" :class="params.relation ? 'c-black' : 'c-lightgrey'">
        ...
        <button class="next-step-button fs-44 fw-500 c-white mt-48" @click="handleNextStepClick">
          下一步
        </button>
      </template>
    </view>
    ...
    <template v-if="hasPhoneNumber">
      ...
      <view class="footer bg-white">
        <button class="add-button fs-44 fw-500 c-white m-0-32" @click="handleBindClick">
          绑定
        </button>
      </view>
    </template>
...
export default {
  ...
  data() {
    return {
      ...
      tenantIndex: -1,
      // 未选择
      unChecked: 'https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-unselect.png',
      // 已选择
      checked: 'https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-selected.png'
    }
  },
  onLoad(options) {
