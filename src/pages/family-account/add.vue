<template>
  <view class="add">
    ...
    <image
      class="background"
      mode="scaleToFill"
      src="http://192.168.1.187:10088/static/family-account/bg-support-bind-success.png"
    />
    ...
    <view class="card bg-white m-0-32 br-24">
      ...
      <template v-if="hasPhoneNumber">
        ...
        <template v-if="canBind">
          ...
          <view class="account flex-h mt-16 br-8" v-for="(item, index) in list1" :key="index">
            ...
            <image
              class="stamp"
              mode="scaleToFill"
              src="http://192.168.1.187:10088/static/family-account/icon-support-stamp-1.png"
            />
          </view>
        ...
        <view class="flex-v flex-c-c m-80-0" v-else>
          <text class="tips fs-36 c-black">您还没有添加赡养扶养关系，添加后才可绑定亲友</text>
  ...
  export default {
    ...
    data() {
      return {
        ...
        tenantIndex: -1,
        // 未选择
        unChecked: 'http://192.168.1.187:10088/static/common/icon-common-unselect.png',
        // 已选择
        checked: 'http://192.168.1.187:10088/static/common/icon-common-selected.png',
      };
    },
    onLoad(options) {
    ...
    methods: {
      ...
      handleBindClick() {
        ...
        if (this.canBind && this.list1.length === 0 && this.list2.length === 0) {
          showToast('暂无可绑定对象');
          return;
        } else if ((this.list1.length !== 0 || this.list2.length !== 0) && arr.length === 0) {
          showToast('请选择要绑定的亲友');
          return;
