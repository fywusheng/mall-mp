<template>
  <view class="index">
    ...
    <!-- #ifdef MP-ALIPAY -->
    <navigation-bar title="亲情账号" title-color="#ffffff" :alpha="1"
    ...
    <view class="list flex-h">
      <view class="item flex-v flex-c-s" v-for="(item, index) in list" :key="index"
        @click="changeFamily(index)">
        <view class="img-box flex-h">
          <image class="avatar animated"
            src="https://ggllstatic.hpgjzlinfo.com/static/family-account/icon-young-man2x.png"
            mode="scaleToFill" :class="{ 'avatar--selected': index === selectedAccountIndex }" />
        </view>
        <text class="relation fs-32 c-black br-4">{{ item.relationRemark }}</text>
        <text class="name fs-36 c-white">
          {{ item.userName }}
        </text>
        <image v-if="index === selectedAccountIndex" class="triangle"
    ...
    <view class="card br-16 flex-v">
      ...
      <template v-if="hasBinded">
        <view class="tab-bar flex-h flex-c-c">
          <text class="tab flex-1 fs-48 animated" :class="selectedTabIndex === 0 ? 'c-primary' : ''"
            @click="handleTabClick(0)">
            亮证
          </text>
          <text class="tab flex-1 fs-48 animated" :class="selectedTabIndex === 1 ? 'c-primary' : ''"
            @click="handleTabClick(1)">
            扫一扫
          </text>
          <text class="tab flex-1 fs-48 animated" :class="selectedTabIndex === 2 ? 'c-primary' : ''"
            @click="handleTabClick(2)">
            老年码
          </text>
          <view class="tab-background br-36 bg-white animated" :class="tabBackgroundClass" />
        </view>

        <view class="electronic-certificate flex-v flex-c-c flex-1" v-if="selectedTabIndex === 0">
          <!-- 已领卡 -->
          <view v-if="haveCard" class="oldcard">
            <view class="certificate-card">
              <image class="bg" mode="scaleToFill"
              ...
              <view class="main flex-h ">
                <view class="main-l flex-1">
                  ...
                  <view class="main-l-item flex-h flex-c-s">
                    ...
                    <view class="main-l-item-r">{{ info.birthday | dateFilter }}</view>
                  </view>
                  <view class="main-l-item flex-h flex-c-s">
              ...
              <view class="main-bt flex-h flex-c-s">
                ...
                <view class="main-bt-right"> {{ info.residentialAddress }}</view>
              </view>
            ...
            <view class="certificate-card  mt-24">
              ...
              <view class="main flex-h ">
                <view class="main-l flex-1">
                  <view class="main-l-item flex-h flex-c-s">
                    ...
                    <view class="main-l-item-r"> {{ info.emergencyContact }}</view>
                  </view>
                  <view class="main-l-item flex-h flex-c-s">
              ...
              <view class="main flex-h lastmain">
                <canvas canvas-id="qr-code-copy" id="qr-code-copy" class="qr-code" />

                <view class="main-l flex-1"> </view>
                <view class="main-r ">
                  <view class="main-r-item flex-h flex-c-s">
                    <view class="main-r-item-l">发证机关</view>
                    <view class="main-r-item-r">{{ info.licenceAuthority }}</view>
                  </view>
                  <view class="main-r-item flex-h flex-c-s">
                    <view class="main-r-item-l">发证日期</view>
                    <view class="main-r-item-r">{{ info.licenceDate | dateFilter }}</view>
                  </view>
                </view>
              </view>
          ...
          <view v-if="!haveCard && authState == 6" class="have-card flex-v flex-c-c">
            ...
            <view class="btn-box mt-60 mb-32">
              <button class="agree-button fs-40 fw-bold c-white" @click="handleAgreeClick">
                同意协议并授权
              </button>
            </view>
            <view class="arguments">
              查看<text class="c-blue" @click="handleUserAgreementClick(2)">《用户授权协议》</text>
              <!-- <text class="c-blue">《电子老年人证用户协议》</text> -->
              <text class="c-blue"
                @click="handleUserAgreementClick(3)">《人脸识别认证服务须知》</text>，授权电子老年人证获取你的姓名、手机号、身份证用于实名领卡，并经由国家老龄服务平台上传您的相片、姓名、社会保障号码信息以完成实人认证。
            </view>
          </view>
        ...
        <view class="qr-code flex-1" v-if="selectedTabIndex === 2">
          ...
          <text class="row fs-36 fw-400 pl-40 mt-32" v-if="codeInfo.userName">
            姓&nbsp;&nbsp;名：{{codeInfo.userName | nameFilter}}
          </text>
          <text class="row fs-36 fw-400 pl-40 mt-32" v-if="codeInfo.idNo">
            身份证：{{codeInfo.idNo | idCardNumberFilter}}
          </text>
          <view class="code-area flex-v flex-c-c bg-white br-16 p-32">
            <canvas canvas-id="bar-code" id="bar-code" class="bar-code-canvas"
              @click="showsBarCode = true" />
            <canvas canvas-id="qr-code" id="qr-code" class="qr-code-canvas"
              @click="showsQRCode = true" />
            <view class="flex-h flex-c-c">
              <text class="fs-40 c-lightgrey">每隔1分钟自动</text>
              <button class="button fs-40 c-primary ml-16" hover-class="none"
      ...
      <template v-else>
        <view class="no-bind flex-v flex-c-c bg-white m-20">
          ...
          <view class="warning flex-h">
            <image class="warning-icon mt-4" mode="scaleToFill"
          ...
          <button class="resend-button fs-40 fw-400 c-white"
            :class="{ 'resend-button--disabled': tips }" @click="sendMessage" :disabled="tips">
            重新发送
          </button>
    ...
    <view class="preview-bar-code bg-white flex-h flex-c-c"
      :style="{ display: showsBarCode ? 'flex' : 'none' }" @click="showsBarCode = false">
      <canvas canvas-id="bar-code-big" id="bar-code-big" class="image" />
    </view>
    <view class="preview-qr-code bg-white flex-h flex-c-c"
      :style="{ display: showsQRCode ? 'flex' : 'none' }" @click="showsQRCode = false">
