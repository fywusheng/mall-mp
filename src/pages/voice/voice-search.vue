    ...
    <template v-if="!content">
      <section-header title="你可以这么问">
        <view class="refresh flex-h flex-c-c" @click="handleRefreshClick">
          <image class="refresh__icon" mode="scaleToFill"
            src="https://ggllstatic.hpgjzlinfo.com/static/voice/icon-voice-refresh.png" />
          <text class="refresh__text fs-40 c-primary ml-8">换一换</text>
        </view>
      </section-header>
      <view class="search-items flex-h flex-wrap">
        <view class="item flex-h flex-c-c br-32 mr-20 mb-40" v-for="(item, index) in randomExamples"
          :key="index" @click="handleExampleItemClick(item)">
          <image class="item__icon"
            src="https://ggllstatic.hpgjzlinfo.com/static/voice/icon-voice-item.png"
            mode="scaleToFill" />
      ...
      <view class="search-items flex-h flex-wrap">
        <view class="item flex-h flex-c-c br-32 mr-20 mb-40" v-for="(item, index) in hotExamples"
          :key="index" @click="handleExampleItemClick(item)">
          <image class="item__icon"
            src="https://ggllstatic.hpgjzlinfo.com/static/voice/icon-voice-item.png"
            mode="scaleToFill" />
    ...
    <template v-else>
      <view class="chat-view flex-v p-32">
        ...
        <view class="flex-v" v-if="results.length > 0">
          <text class="result fs-40 c-primary mt-16" v-for="(item, index) in results" :key="index"
