  ...
  <view class="identity-info">
    ...
    <view class="form f-s-40">
      <view class="row flex-h flex-c-s p-20-0">
        ...
        <input class="fs-40 c-black flex-1 p-r-48" placeholder="请输入姓名"
      ...
      <view class="row flex-h flex-c-s p-20-0">
        ...
        <input class="fs-40 c-black flex-1 p-r-48" type="idcard" placeholder="请输入身份证号"
      ...
      <view class="row flex-h flex-c-s p-20-0" v-if="scanInfor.type === 'scan'">
        ...
        <!-- #ifdef MP-ALIPAY -->
        <button @click="alipayChose" class="clickChose f-s-40">{{params.city || '请选择所在地区'}}</button>
        <!-- #endif -->
        <!-- #ifdef MP-WEIXIN -->
    ...
    <!-- #ifdef MP-ALIPAY -->
    <button class="next-step-button bg-cancel fs-44 c-white" @click="handleBack">
      关闭
    </button>
    <!-- #endif -->
  ...
  methods: {
    ...
    async handleNextStepClick() {
      if (this.isClick) {
        ...
        this.repeatClick = setTimeout(() => { this.isClick = true }, 3000)
      }
    },
    /**
     * 获取用户信息
