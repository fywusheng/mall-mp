  ...
  <view class="map">
    ...
    <view class="search-view flex-h flex-c-b bg-white p-0-32">
      <view class="search-bar flex-h flex-c-b flex-1 p-0-32">
        <text class="search-bar__text fs-36 flex-1" :class="keyword ? 'c-black' : 'c-lightgrey'" @click="handleSearchBarClick">
      ...
      <button v-if="keyword" class="cancel-button fs-36 c-black ml-24" hover-class="none" @click="handleCancelClick">取消</button>
    </view>
    <button v-if="showsSearchInCurrentArea && keyword && (!off || !showsInfoCard)" class="search-button fs-36 c-black bg-white" @click="handleQuery">
      在此区域搜索
    </button>
    <image class="welfare" src="http://192.168.1.187:10088/static/map/icon-map-welfare.png" @click="handleWelfareClick" mode="scaleToFill" />
    <view v-if="!showsInfoCard" class="actions flex-v">
      <view class="item flex-h flex-c-c bg-white br-16" @click="handleLocateClick">
        <image class="item__icon" src="http://192.168.1.187:10088/static/home/icon-home-locate.png" mode="scaleToFill" />
        <text class="fs-36 c-lightgrey ml-12">定位</text>
      </view>
      <view class="item flex-h flex-c-c bg-white br-16 mt-16" @click="handleRoadClick">
        <image class="icon-road" src="http://192.168.1.187:10088/static/map/road.png" mode="scaleToFill" />
        <text class="fs-36 c-lightgrey ml-12">路线</text>
      </view>
    </view>
    <scroll-view scroll-x="true" :enable-flex="true" :scroll-left="scrollToLeft" scroll-with-animation class="categories flex-h bg-white">
      <view
        ...
        <image
          ...
          :src="
            item.serviceOrder == serviceIndex
              ? 'http://192.168.1.187:10088/static/map/menu-icon-selected' + item.serviceOrder + '.png'
              : 'http://192.168.1.187:10088/static/map/menu-icon' + item.serviceOrder + '.png'
          "
        />
        <text class="item__text fs-40 mt-24 animated" :class="item.serviceOrder == serviceIndex ? 'c-bold' : 'c-lightgrey'">
          {{ item.serviceName }}
        </text>
      </view>
    </scroll-view>
    <view class="swiper-box" :animation="animationData" v-if="showsInfoCard">
      <uni-swiper-dot :info="markers" field="content" :mode="mode">
        <swiper class="height-500" @change="change" @animationfinish="animationfinish" :current="current" :duration="duration">
          <swiper-item v-for="(item, index) in markers" :key="index" :item-id="index" @touchstart="handleTouchstart" @touchend="handleTouchend">
            <view class="swiper-item">
              <view class="map-icon" @click="handleAnimation">
                <image mode="scaleToFill" src="http://192.168.1.187:10088/static/map/icon-map-arrow-upward.png" :class="roateImage"></image>
              </view>
              <view class="info-card flex-v m-0-32 bg-white br-24" @click="handleInfoCardClick(item)">
                <view class="info flex-h">
                  <view class="photo-view flex-v flex-c-s">
                    <image class="photo" mode="scaleToFill" :src="item.scenic_area_photos" v-if="item.scenic_area_photos" />
                    <image
                      class="photo"
                      mode="scaleToFill"
                  ...
                  <view class="text-view flex-v flex-1 ml-16">
                    ...
                    <text class="text fs-36 c-grey mt-8">{{ item.type }} | {{ item.my_distance }}</text>
                    <text class="text fs-36 c-grey mt-8" v-if="item.address">
                      {{ item.address }}
                    </text>
                    <text class="text fs-36 c-grey mt-8" v-if="item.hospital_clinic">特色科室：{{ item.hospital_clinic }}</text>
                    <text class="text fs-36 c-grey mt-8" v-if="!item.address && item.scenic_area_opentime">
                      开放时间：{{ item.scenic_area_opentime }}
                    </text>
                    <text class="text fs-36 c-grey mt-8" v-if="!item.address">特色介绍：{{ item.scenic_area_intro }}</text>
                  </view>
                </view>
                <view class="functions flex-h flex-c-b">
                  <view class="flex-1 flex-h">
                    <view class="item" @click.stop="handleCollected(item)" v-if="item.isCollected == 0 || item.isCollected == 1">
                      <image
                        class="icon"
                        mode="scaleToFill"
                      ...
                      />
                      <image class="icon" mode="scaleToFill" src="http://192.168.1.187:10088/static/map/icon-map-uncollect.png" v-else />
                      <text class="fs-36 c-black mt-8">收藏</text>
                    </view>
                    <view class="item" @click.stop="makeToTel(item)" v-if="showTel(item.scenic_area_tel)">
                      <image class="icon" mode="scaleToFill" src="http://192.168.1.187:10088/static/map/icon-map-make-call.png" />
                  ...
                  <view class="direction flex-h flex-c-c ml-12" @click.stop="handleDirectionClick(item)">
    ...
    <action-sheet ref="actionSheet" title="请选择拨打电话" :items="actionSheetItems" @click="handleActionSheetItemClick" />
  </view>
</template>
...
<script>
  ...
  var page;
  export default {
    components: { uniSwiperDot, ActionSheet },
    data() {
      return {
        // 当前位置信息
              ...
              res.pois.map((item, index) => {
                if (this.serviceIndex == 0) {
                  ...
                  arry.push({
                    ...
                    latitude: item.location.split(',')[1] - 0,
                    longitude: item.location.split(',')[0] - 0,
                    iconPath: 'http://192.168.1.187:10088/static/map/marker-icon' + this.baseList[this.serviceIndex].serviceOrder + '.png',
                    width: 35,
                    height: 36,
                    name: item.name,
                    scenic_area_tel: item.tel,
      ...
      handleMapRegionChange(e) {
        ...
        mapConext.getRegion({
          success: (res) => {
            ...
            // 获取当前屏幕半径
            const radius = (this.getDistance(lat, res.southwest.longitude, lat, res.northeast.longitude) / 2).toFixed(0);
            ...
            mapConext.getCenterLocation({
              success: (res) => {
                ...
                // if (this.showOnce) {
                this.handleCategoryClick(this.serviceIndex, res.latitude, res.longitude, true, radius);
