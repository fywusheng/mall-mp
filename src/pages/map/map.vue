<template>
  <view class="map">
    <map ref="map" id="map" :show-scale="true" class="map-view" :latitude="centerLocation.latitude"
      :longitude="centerLocation.longitude" :show-location="true" :include-points="list"
      :markers="markers" :max-scale="maxScale" :enable-3D="false"
      @regionchange="handleMapRegionChange" @tap="handleMapClick" @markertap="handleMarkerClick" />
    <view class="search-view flex-h flex-c-b bg-white p-0-32">
      <view class="search-bar flex-h flex-c-b flex-1 p-0-32">
        <text class="search-bar__text fs-36 flex-1" :class="keyword ? 'c-black' : 'c-lightgrey'"
          @click="handleSearchBarClick">
          {{ keyword || "搜索地点、美食、景点、医院等" }}
        </text>
        <image v-if="keyword" mode="scaleToFill" class="search-bar__icon"
          src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-input-clear.png"
          @click="handleCancelClick" />
        <template v-else>
          <view class="search-bar__line" />
          <image mode="scaleToFill" class="search-bar__voice ml-16"
    ...
    <button v-if="showsSearchInCurrentArea && keyword && (!off || !showsInfoCard)"
      class="search-button fs-36 c-black bg-white" @click="handleQuery">
      在此区域搜索
    </button>
    <image class="welfare" src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-welfare.png"
      @click="handleWelfareClick" mode="scaleToFill" />
    <view v-if="!showsInfoCard" class="actions flex-v">
      <view class="item flex-h flex-c-c bg-white br-16" @click="handleLocateClick">
        <image class="item__icon"
          src="https://ggllstatic.hpgjzlinfo.com/static/home/icon-home-locate.png"
          mode="scaleToFill" />
        <text class="fs-36 c-lightgrey ml-12">定位</text>
      </view>
      <view class="item flex-h flex-c-c bg-white br-16 mt-16" @click="handleRoadClick">
        <image class="icon-road" src="https://ggllstatic.hpgjzlinfo.com/static/map/road.png"
          mode="scaleToFill" />
        <text class="fs-36 c-lightgrey ml-12">路线</text>
      </view>
    </view>
    <scroll-view scroll-x="true" :enable-flex="true" :scroll-left="scrollToLeft"
      scroll-with-animation class="categories flex-h bg-white">
      <view class="item menu-item flex-v flex-c-c mb-4" v-for="(item, index) in categories"
        :key="index" @click="selectedIndex = index,handleCategoryClick(item.serviceOrder)">
        <image class="item__icon" mode="scaleToFill"
          :src="item.serviceOrder == serviceIndex ? 'https://ggllstatic.hpgjzlinfo.com/static/map/menu-icon-selected'+item.serviceOrder+'.png' : 'https://ggllstatic.hpgjzlinfo.com/static/map/menu-icon'+item.serviceOrder+'.png'" />
        <text class="item__text fs-40 mt-24 animated"
    ...
    <view class="swiper-box" :animation="animationData" v-if="showsInfoCard">
      <uni-swiper-dot :info="markers" field="content" :mode="mode">
        <swiper class="height-500" @change="change" @animationfinish="animationfinish"
          :current="current" :duration="duration">
          <swiper-item v-for="(item, index) in markers" :key="index" :item-id="index"
            @touchstart="handleTouchstart" @touchend="handleTouchend">
            <view class="swiper-item">
              <view class="map-icon" @click="handleAnimation">
                <image mode="scaleToFill"
                  src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-arrow-upward.png"
                  :class="roateImage"></image>
              </view>
              <view class="info-card flex-v m-0-32 bg-white br-24"
                @click="handleInfoCardClick(item)">
                <view class="info flex-h">
                  <view class="photo-view flex-v flex-c-s">
                    <image class="photo" mode="scaleToFill" :src="item.scenic_area_photos"
                      v-if="item.scenic_area_photos" />
                    <image class="photo" mode="scaleToFill"
                  ...
                  <view class="text-view flex-v flex-1 ml-16">
                    ...
                    <text class="text fs-36 c-grey mt-8">{{ item.type }} |
                      {{ item.my_distance }}</text>
                    <text class="text fs-36 c-grey mt-8" v-if="item.address">{{
                      item.address
                    }}</text>
                    <text class="text fs-36 c-grey mt-8"
                      v-if="item.hospital_clinic">特色科室：{{ item.hospital_clinic }}</text>
                    <text class="text fs-36 c-grey mt-8"
                      v-if="!item.address && item.scenic_area_opentime">开放时间：{{ item.scenic_area_opentime }}</text>
                    <text class="text fs-36 c-grey mt-8"
                      v-if="!item.address">特色介绍：{{ item.scenic_area_intro }}</text>
                  </view>
                </view>
                <view class="functions flex-h flex-c-b">
                  <view class="flex-1 flex-h">
                    <view class="item" @click.stop="handleCollected(item)"
                    ...
                    <view class="item" @click.stop="makeToTel(item)"
                      v-if="showTel(item.scenic_area_tel)">
                      <image class="icon" mode="scaleToFill"
                        src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-make-call.png" />
                  ...
                  <view class="direction flex-h flex-c-c ml-12"
    ...
    <action-sheet ref="actionSheet" title="请选择拨打电话" :items="actionSheetItems"
    ...
    return {
      ...
      baseList: [
        ...
        },
        {
          ...
          des: '医院',
          amapDes:
            '医疗保健服务场所|综合医院|专科医院|诊所|急救中心|疾病预防机构',
          prepareIndex: 1
        },
        {
          serviceOrder: '7',
  ...
  methods: {
                    ...
                    if (conditions) {
                      {
                        ...
                        return arry.push({
                          ...
                          iconPath:
                            'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon' +
                          ...
                          scenic_area_rank:
                            typeof item.biz_ext.level == 'string'
                              ? item.biz_ext.level
                              : ''
                        })
                      }
                      ...
                      return arry.push({
                        ...
                        latitude: item.location.split(',')[1] - 0,
                        longitude: item.location.split(',')[0] - 0,
                        iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon0.png',
                        width: 35,
                        height: 36,
                        name: item.name,
    ...
    handleMapRegionChange(e) {
      ...
      mapConext.getRegion({
        success: (res) => {
          ...
          const lat = (
            (res.southwest.latitude + res.northeast.latitude) /
            2
          ).toFixed(6)
          // 获取当前屏幕半径
          const radius = (
    ...
    makeToTel(item) {
      ...
      if (item.scenic_area_tel) {
        const phoneNumber = item.scenic_area_tel
          .replace(/\//g, ',')
          .replace(/\;/g, ',')
    ...
    async handleCategoryClick(
      ...
      api.getEcOrgOfficeInfoByPoint({
        data: {
          ...
          distance: drag
            ? radius
            : Number(this.baseList[this.serviceIndex].distance / 1000),
          size: 25
        },
        success: (data) => {
          this.setMarkers = []
            ...
            switch (item.orgOfficeType) {
              case '景点':
                ...
                obj = {
                  ...
                  iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon5.png',
                  width: 35,
                  height: 36,
                ...
                if (item.orgOfficeInfoDetail) {
                  ...
                  if (area.scenic_area_photos) {
                    obj['scenic_area_photos'] =
                      area.scenic_area_photos.split(',')[0]
                  }
                }
              ...
              case '医院':
                ...
                obj = {
                  ...
                  latitude: item.orgOfficeLat,
                  longitude: item.orgOfficeLon,
                  iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon6.png',
                  width: 35,
                  height: 36,
                  name: item.orgOfficeName,
                  orgOfficeInfoDetail: item.orgOfficeInfoDetail,
              ...
              default:
                // console.log('执行报错点13')
                let iconPath = 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon0.png'
                const iconp = {
                  助餐点: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon1.png',
                  日间照料: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon2.png',
                  商超: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon3.png',
                  美食: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon4.png',
                  药店: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon7.png',
                  住宿: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon8.png',
                  政务服务: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon9.png',
                  养老院: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon10.png'
                }
                ...
                if (item.orgOfficeInfoDetail) {
                  ...
                  if (area.scenic_area_photos) {
                    ...
                    obj['scenic_area_photos'] =
                      area.common_service_photos.split(',')[0]
                  }
                }
            ...
            api.regeoMap(
              ...
              },
              {
                success: (resp) => {
                  ...
                  if (
                    typeof resp.regeocode.addressComponent.city === 'string'
                  ) {
    ...
    .info {
      .photo-view {
        ...
        .photo-rank {
          ...
          width: 100%;
          height: 50rpx;
          background: url('https://ggllstatic.hpgjzlinfo.com/static/map/photo-rank2x.png') no-repeat;
