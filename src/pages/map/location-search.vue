<template>
  <view class="location-search">
    ...
    <view class="list"
      ...
      <view class="item flex-h bg-white"
        ...
        <view class="ml-24 flex-v flex-1"
          ...
          <text class="distance fs-36 c-lightgrey mb-8"
      ...
      <view class="list bg-white mt-12">
        ...
        >
          <view class="flex-h flex-c-s">
            <image class="address-icon" src="https://ggllstatic.hpgjzlinfo.com/static/logo.png" />
    ...
    <template v-else>
      <view class="current-location m-32 p-0-24 flex-h flex-c-s bg-white br-16"
      ...
      <view v-if="searchHistories.length > 0"
            class="list history bg-white br-16">
        <view class="header flex-h flex-c-s p-0-24">
          <text class="fs-36 c-black">历史选点</text>
        </view>
        <view class="item p-24 flex-v"
          ...
          <view class="flex-h flex-c-s">
            <image class="address-icon"
                   mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/logo.png" />
            <text class="address-text fs-36 c-black ml-16">
              {{ item.name }}
            </text>
          </view>
        ...
        <view class="footer flex-h flex-c-c"
...
export default {
  ...
  watch: {
    ...
    async keyword(newVal, oldVal) {
      let ownMarkers = await this.getAroundApi()
      let gdMarkers = await this.getAround()

      // 获取数据后先去重后再进行排序
      let arry = [...ownMarkers, ...gdMarkers]
      ...
      this.list.map((item, index) => {
        let arr = this.list[index]['name'].split(this.keyword)
        let nameArr = []
        arr.map((item, index) => {
  ...
  methods: {
    ...
    handleListItemClick(index) {
      const item = {
        ...
        iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/marker-icon4.png',
        width: 47,
        height: 52,
      }
