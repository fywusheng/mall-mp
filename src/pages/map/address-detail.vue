  ...
  <view class="address-detail">
    ...
    <view class="footer bg-white">
      <view class="functions flex-h p-16-0">
        <view class="flex-v flex-c-c flex-1" @click="handleCollect">
          <image class="icon"
            ...
            mode="scaleToFill" v-if="dataDetail.isCollected == 1" />
          <image class="icon"
        ...
        <view class="flex-v flex-c-c flex-1" @click="handleShareClick(dataDetail)">
          <image class="icon" src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-share.png"
            mode="scaleToFill" />
      ...
      <view class="flex-h flex-c-c mt-16">
        <button class="button-1 fs-44 c-primary m-0-16" hover-class="none"
          @click="handleDirectButtonClick">
          导航
        </button>
        <button class="button-2 fs-44 c-white m-0-16" hover-class="none"
      ...
      <view class="share-pop pt-32">
        ...
        <view class="_list">
          <button class="share_icon " open-type="share">
            <image class="_img"
              src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-wechat.png" mode="" />
            <text>微信好友</text>
          </button>
          <!-- #ifdef MP-ALIPAY -->
          <button class="share_icon" open-type="share">
            <image class="_img" src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-qq.png"
              mode="scaleToFill" />
            <text>QQ好友</text>
          </button>
          <button class="share_icon" open-type="share">
          ...
          <button class="share_icon" @click="handleCopyClick">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-link.png" class="_img"
              mode="" />
            <text>复制链接</text>
          </button>
        </view>
  ...
  onLoad(e) {
            ...
            success: (data) => {
              ...
              if (data.orgOfficeInfoDetail) {
                ...
                if (detail.scenic_area_photos) {
                  this.dataDetail['photoShop'] = [
                    ...detail.scenic_area_photos.split(',')
                  ]
                } else if (detail.common_service_photos) {
                  this.dataDetail['photoShop'] = [
                    ...detail.common_service_photos.split(',')
                  ]
                }
                if (detail.scenic_area_rank) {
                  this.dataDetail['scenic_area_rank'] = detail.scenic_area_rank
                } else if (detail.common_service_rank) {
                  this.dataDetail['scenic_area_rank'] =
                    detail.common_service_rank
                }
                if (detail.scenic_area_intro) {
                  this.dataDetail['scenic_area_intro'] =
                    detail.scenic_area_intro
                } else if (detail.common_service_intro) {
                  this.dataDetail['scenic_area_intro'] =
                    detail.common_service_intro
                }
                console.log
                if (detail.common_service_opentime) {
                  this.dataDetail['scenic_area_opentime'] =
                    detail.common_service_opentime
                } else if (detail.scenic_area_opentime) {
                  this.dataDetail['scenic_area_opentime'] =
                    detail.scenic_area_opentime
                }
                ...
                if (detail.scenic_area_tel) {
                  ...
                  const phoneNumber = detail.common_service_tel.replace(
                    /\//g,
                    ','
                  )
                  phoneNumber.split(',').forEach((item) => {
    ...
    eventChannel.on('didOpenPageFinish', (data) => {
      ...
      if (data.item.orgOfficeInfoDetail) {
        const detail = JSON.parse(
          data.item.orgOfficeInfoDetail.replace(/\s*/g, '')
        )
        this.dataDetail['infoDetail'] = detail
        this.dataDetail['photoShop'] = []
        if (detail.scenic_area_photos) {
  ...
  methods: {
    ...
    handleCopyClick() {
      ...
      if (this.type === 'attraction') {
        url = `https://ggll.hpgjzlinfo.com/#/map/scenic-details/${this.orgOfficeid}?isShare=1`
      } else if (this.type === 'hospital') {
        url = `https://ggll.hpgjzlinfo.com/#/map/hospital-details/${this.orgOfficeId}?isShare=1`
      }
