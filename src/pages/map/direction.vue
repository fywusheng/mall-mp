<template>
  <view class="direction">
    <view class="header flex-v bg-white">
      <view class="origin-destination-info flex-h flex-c-b pl-32">
        <view class="indicator flex-v flex-c-b">
          ...
          <image class="dash-line"
                 src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-dash-line.png"
                 mode="scaleToFill" />
          <view class="red-dot br-8" />
        </view>
        <view class="info flex-1 ml-16">
          <text :class="['row','fs-36','c-black','p-0-24','br-16','animated',isInterchanged ? 'bottom' : 'top',showEmpty ? 'selectable' : '']"
                @click="handleOriginClick">
            {{ isInterchanged ? (destination.name?destination.name:'') : (origin.name?origin.name:'') }}
          </text>
          <text :class="['row','fs-36','c-black','p-0-24','br-16','animated',isInterchanged ? 'top' : 'bottom',showEmpty ? '' : 'selectable']"
                @click="handleDestinationClick">
            {{ isInterchanged ? (origin.name?origin.name:'') :(destination.name?destination.name:'') }}
          </text>
        </view>
        <image class="interchange-icon m-0-48"
               mode="scaleToFill"
               src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-interchange.png"
               @click="handleInterchangeClick" />
      </view>
      <view class="trip-mode flex-h">
        <view class="mode flex-v flex-c-c flex-1"
          ...
          <view class="flex-h flex-c-c flex-1">
            <image v-if="index === selectedIndex"
                   ...
                   :src="item.iconPath"
                   mode="scaleToFill" />
            <text class="text fs-40 animated"
                  :class="index === selectedIndex ? 'c-primary' : 'c-lightgrey'">
              {{ item.name }}
            </text>
          </view>
          <text class="duration fs-36 animated duration-size"
      ...
      <view class="tab">
        <myTabbar :modelData="modelData"
                  :initIndex="initIndex"></myTabbar>
      </view>
    </view>
    <map v-if="selectedIndex !== 1 && showEmpty"
         class="map-view"
         ref="map"
         id="map"
    ...
    <view v-else
          ...
          <view class="info flex-v">
            ...
            <view class="line-info flex-h flex-c-s flex-wrap">
              <template v-for="(busline, bIndex) in item.buslines">
                <text class="line fs-40 fw-bold c-black"
                      :key="bIndex">
                  {{ busline }}
                </text>
                <image v-if="bIndex !== item.buslines.length - 1"
      ...
      <template v-if="buslines.length <= 0 && showEmpty && !isDistanceLong">
        <view class="empty flex-v flex-c-c">
          ...
          <text class="fs-40 c-black mt-16">
            步行{{ tripModes[3].duration }}
          </text>
          <text class="fs-36 c-black mt-16">路线距离较短，建议步行前往</text>
          <button class="button fs-40 c-primary mt-48"
                  hover-class="none"
    ...
    <view v-if="selectedIndex !== 1 && showEmpty"
          class="footer flex-v flex-c-c bg-white">
      <view class="text flex-h"
            v-if="tripModes[selectedIndex].duration != '暂无'">
        <text class="fs-36 c-lightgrey">
          全程{{ tripModes[selectedIndex].distance }}，预计
        </text>
      ...
      <view class="flex-button"
        ...
        <button class="button1 fs-44 c-white mt-32"
                @click="handleCarButtonClick">
          打车
        </button>
        <button class="button2 fs-44 c-white mt-32"
                @click="handleDirectButtonClick">
          导航
        </button>
      </view>
      <view v-if="selectedIndex != 0">
        <button class="button fs-44 c-white mt-32"
                @click="handleDirectButtonClick">
          使用地图导航
        </button>
      </view>
    ...
    <view class="fiex-map"
          v-if="showEmpty && selectedIndex != 1">
      <view class="flex-map"
            @click="handlePosition(1)">
        <image src="https://ggllstatic.hpgjzlinfo.com/static/map/location2x.png"
               mode="scaleToFill"/>
        <text>商户位置</text>
      </view>
      <view class="flex-map"
            @click="handlePosition(2)">
        <image src="https://ggllstatic.hpgjzlinfo.com/static/home/icon-home-locate.png"
               mode="scaleToFill"/>
        <text>当前定位</text>
      </view>
    </view>
...
export default {
  ...
  data() {
    return {
      ...
      tripModes: [
        {
          iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-trip-mode-driving.png',
          name: '驾车',
          distance: '',
          duration: '',
        },
        {
          iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-trip-mode-busing.png',
          name: '公交',
          distance: '',
          duration: '',
        },
        {
          iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-trip-mode-cycling.png',
          name: '骑行',
          distance: '',
          duration: '',
        },
        {
          iconPath: 'https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-trip-mode-walking.png',
          name: '步行',
          distance: '',
          duration: '',
        },
      ...
      uni.request({
        ...
        success: (res) => {
          ...
          walkItem = this.buslines.reduce((p, v) => {
            ...
            return Number(p.walking_distance) <= Number(v.walking_distance)
              ? p
              : v
          })
