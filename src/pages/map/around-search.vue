<template>
  <view class="around-search" @click="handleClose">
    <view class="header bg-white">
      ...
      <view class="filters flex-h">
        <view class="flex-width">
          <min-collapse ref="collapse1" :list="[]" :desc="collapseDesc1" textDirection="left"
            @clickTab="clickTab(1)">
            <view class="around">
              <view class="flex-width">
              ...
              <view class="flex-width">
                <view :class="['minate-item', disCurrent == index ? 'hight-line' : '']"
                  v-for="(item,index) in distance" :key="index" @click="clickRound(index)">
                  <text>{{item.name}}</text>
                </view>
              </view>
            </view>
        ...
        <view class="flex-width">
          <min-collapse ref="collapse2" :list="categories" :desc='collapseDesc2'
            textDirection="center" @clickTab="clickTab(2)" @handleItemClick="handleItemClick">
          </min-collapse>
        </view>
        <view class="flex-width">
          <min-collapse ref="collapse3" :list="[]" :desc='collapseDesc3' textDirection="right"
            @clickTab="clickTab(3)">
            <view class="down-collapse">
              <view class="collapse-item">
                <view class="collapse-title">优惠</view>
                <view class="flex-list">
                  <view :class="['list-item',disCountCurrent == index ? hightLine : '']"
              ...
              <view class="collapse-item">
                ...
                <view class="flex-list">
                  <view :class="['list-item',serviceCurrent == index ? hightLine : '']"
                    v-for="(item, index) in serverList" :key="index" @click="choiceClick(2,index)">
                    {{item.name}}
                  </view>
                </view>
              </view>
              <view class="btns">
                ...
                <view class="flex-h flex-1 flex-c-c fs-40 btn sure" @click="confirmClick">确定</view>
              </view>
            </view>
          </min-collapse>
        </view>
    ...
    <view class="list">
      <view class="item flex-h" v-for="(item,index) in aroundList" :key="index"
        @click="handleMapClick(item)">
        <image class="image" :src="item.scenic_area_photos" v-if="item.scenic_area_photos"
          mode="scaleToFill" />
        <image class="image"
          src="https://ggllstatic.hpgjzlinfo.com/static/home/image-home-article-default.png" v-else
          mode="scaleToFill" />
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.typeCurrent == 0) {
          this.categories.map((item, index) => {
            if (index > 0) {
              listTypeName =
                listTypeName + this.baseList[Number(item.serviceOrder)].amapDes
            }
          })
            ...
            data.forEach((item, index) => {
              ...
              switch (item.orgOfficeType) {
                case '景点':
                  ...
                  if (
                    this.filterName &&
                    this.filterName === item.orgOfficeName
                  ) {
                    break
                  }
                  obj = {
