  ...
  <view class="search">
    <view class="search-view flex-h flex-c-b bg-white p-0-32">
      <view class="search-bar flex-h flex-c-b flex-1 p-0-32">
        <input v-model="keyword" :adjust-position="false"
          class="search-bar__input fs-36 c-black flex-1" placeholder="搜索地点、美食、景点、医院等"
          :maxlength="maxlength" />
        <image v-if="keyword" mode="scaleToFill" class="search-bar__icon ml-8"
      ...
      <button class="cancel-button fs-36 c-black ml-24" hover-class="none"
    ...
    <template v-if="keyword">
      <view v-if="keyword" class="flex-h flex-c-s p-0-32 bg-white" @click="handleSearchClick">
        <!-- <image class="search-info__icon" src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-search-around.png" />
      ...
      <view class="list">
        <view class="item flex-h bg-white" v-for="(item, index) in list" :key="index">
          <image class="icon mt-12" mode="scaleToFill"
            src="https://ggllstatic.hpgjzlinfo.com/static/map/location2x.png" />
          <view class="ml-24 flex-v flex-1">
            ...
            <text class="distance fs-36 c-lightgrey mb-8" v-if="item.my_distance != 0">
              距离您：{{ item.my_distance }}
            </text>
            <button class="button fs-36 c-black" hover-class="none"
    ...
    <template v-else>
      <view class="section m-32" v-if="searchHistories.length > 0">
        <view class="section-header flex-h flex-c-b">
          ...
          <view class="flex-h flex-c-c" @click="handleClearClick">
            <image class="clear-icon" mode="scaleToFill"
              src="https://ggllstatic.hpgjzlinfo.com/static/map/delete2x.png" />
            <button class="fs-36 c-lightgrey ml-8" hover-class="none">
              清空
            </button>
          </view>
        </view>
        <view class="list flex-h flex-wrap">
          <text class="item bg-white bt-8 mr-24 mt-24" v-for="(item, index) in searchHistories"
            :key="index" @click="handleSearchHistoryItemClick(item)">
            {{ item }}
          </text>
        </view>
      </view>
      <view class="section m-32">
        ...
        <view class="list flex-h flex-wrap">
          <text class="item bg-white bt-8 mr-24 mt-24" @click="handleHotSearchItemClick('北京')">
            北京
          </text>
          <text class="item bg-white bt-8 mr-24 mt-24" @click="handleHotSearchItemClick('故宫博物馆')">
            故宫博物馆
          </text>
          <text class="item bg-white bt-8 mr-24 mt-24" @click="handleHotSearchItemClick('八达岭长城')">
            八达岭长城
          </text>
          <text class="item bg-white bt-8 mr-24 mt-24" @click="handleHotSearchItemClick('骏豪中央广场')">
            骏豪中央广场
          </text>
        </view>
      </view>
    </template>
    <voice-input :show="showsVoiceInput" @change="handleVoiceInputChange"
      @end="handleVoiceInputFinish" />
    <uni-popup ref="popup" type="bottom">
      ...
      <view class="bottom-view flex-v flex-c-c">
        ...
        <image class="bottom-view__image" mode="scaleToFill"
...
export default {
  components: { VoiceInput, uniPopup },
  data() {
    return {
      // 搜索关键字
      keyword: '',
        ...
        },
        {
          success: (data) => {
            ...
            this.list.forEach((item, index) => {
              ...
              if (
                this.list[index]['typecode'].indexOf('09') == 0 ||
                this.list[index]['typecode'].indexOf('09') == 7
              ) {
                this.list[index]['type'] = '医院'
              } else if (
                this.list[index]['typecode'].indexOf('11') == 0 ||
                this.list[index]['typecode'].indexOf('11') == 7
              ) {
