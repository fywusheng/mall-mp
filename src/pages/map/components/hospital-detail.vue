  ...
  <view class="hospital-detail">
    <view class="section flex-v p-32">
      ...
      <text class="fs-36 fw-500 c-black mt-24"
        >性质：{{ dataDetail.infoDetail.hospital_nature }}</text
      >
      ...
      <view class="actions flex-h flex-c-s">
        <view class="action flex-h flex-c-c mr-20 p-0-20" @click="handleMap">
          <image class="icon"  mode="scaleToFill" src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-location.png" />
          <text class="text fs-36 c-lightgrey ml-8">位置</text>
        </view>
        <view class="action flex-h flex-c-c mr-20 p-0-20" @click="handlePhone">
          <image class="icon"  mode="scaleToFill" src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-make-call-full.png" />
    ...
    <view class="section flex-v p-32">
      ...
      <view class="mt-24">
        <text class="content fs-36 c-black">
          {{
            textShow
              ? (dataDetail.infoDetail.hospital_intro ? dataDetail.infoDetail.hospital_intro : '暂无')
              : (dataDetail.infoDetail.hospital_intro ? getString(dataDetail.infoDetail.hospital_intro) : '暂无') 
          }}
        </text>
        <text v-if="dataDetail.infoDetail.hospital_intro.length > 100" class="fs-36" style="color: #0059FF" @click="handleText()">
          {{ textShow ? "收起" : "...全文" }}
        </text>
      </view>
    </view>
    <view class="section flex-v p-32">
      <text class="content fs-36 c-black" v-if="dataDetail.infoDetail.hospital_create_time">{{`创建时间：${dataDetail.infoDetail.hospital_create_time}`}}</text>
      <text class="content fs-36 c-black" v-if="dataDetail.infoDetail.hospital_bed">{{`床位数量：${dataDetail.infoDetail.hospital_bed}`}}</text>
      <text class="content fs-36 c-black" v-if="dataDetail.infoDetail.hospital_clinic_numbers">{{`科室数目：${dataDetail.infoDetail.hospital_clinic_numbers}`}}</text>
      <text class="content fs-36 c-black" v-if="dataDetail.infoDetail.hospital_clinic">{{`特色科室：${dataDetail.infoDetail.hospital_clinic}`}}</text>
      <text class="content fs-36 c-black" v-if="dataDetail.infoDetail.hospital_website">{{`网址：${dataDetail.infoDetail.hospital_website}`}}</text>
    </view>
    <action-sheet
      ref="actionSheet"
      title="请选择拨打电话"
