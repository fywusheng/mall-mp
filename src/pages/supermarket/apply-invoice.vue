<template>
  <view class="pages"
    ...
    <view class="item pay-info br-16" :style="{marginBottom:(info.invoiceStatus == '0' && info.transactionStatus == '1')? '180rpx':''}">
      <view class="content">
        ...
        <view class="msg-item flex-h fs-32" >
          ...
          <view class="msg-right"
                v-if="infoHeader.payeeName">{{infoHeader.payeeName}}</view>
        </view>
        <view class="msg-item flex-h fs-32">
          <view class="msg-left">收票人手机号：</view>
          <view class="msg-right"
                v-if="infoHeader.payeePhone">{{infoHeader.payeePhone}}</view>
        </view>
        <view class="msg-item flex-h fs-32">
          <view class="msg-left">收票人邮箱:</view>
          <view class="msg-right"
                v-if="infoHeader.payeeEmail">{{infoHeader.payeeEmail}}</view>
        </view>
...
export default {
  ...
  methods: {
    ...
    handleChoiceClick(){
      uni.navigateTo({
         url: '/pages/supermarket/company-list?select=1&invoiceHeaderId='+ this.infoHeader.invoiceHeaderId
      });
    },
    //抬头信息回显
    handleSetdata(data){
    ...
    handleSubmitClick(){
      ...
      const data = { 
          ...
          name:this.infoHeader.invoiceType==='P'? this.infoHeader.invoiceHeaderName:this.infoHeader.payeeName, //发票抬头；抬头类型为企业时，填企业名称，类型为个人时，填写个人名字
          taxpayerIdentityNumber:this.infoHeader.invoiceType==='P'?this.infoHeader.taxNo : '', //税号；当抬头类型为企业时，必填
      ...
      if(this.info.orderSource ===4){
        ...
        api.submitInvoiceByType({
          ...
          success:(res)=>{
            console.log(res)
            uni.reLaunch({
              url: '/pages/supermarket/apply-success?orderId='+this.info.orderId +'&invoiceId='+res
            });
          }
        })
      }else{
