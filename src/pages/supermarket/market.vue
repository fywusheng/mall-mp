<template>
  <view class="market">
    ...
    <view class="page-header mt-16">
      <image class="bg"
             src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/bg-card.png"
             mode="scaleToFill" />
      <view class="market-card bg-white br-24">
        ...
        <view class="flex-h address" @click="handleDirectionClick">
          <image class="location"
                 src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/icon-location.png"
                 mode="scaleToFill" />
      ...
      <image class="header-meng"
             src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/card-bottom.png"
             mode="scaleToFill" />
      <view class="discount">
        <image class="img"
               src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/bg-discount.png"
               mode="scaleToFill" />
        <view class="count ">{{info.showDiscount | markrtDiscount}}折</view>
      </view>
    </view>
    <view v-if="otherMarketNum > 1" class="other-store flex-h flex-c-b ml-32 bg-white" @click="handleOtherMarket">  
      <view class="fs-36">查看其他门店地址</view>
      <image class="img"
             src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/icon-arrow.png"
             mode="scaleToFill" />
    </view>
    <view class="grey-bar"></view>
    <view class="page-main bg-white">
      <view class="tab-bar flex-h flex-c-s mb-12">
        <view :class="tabIndex === '0' ? 'active item fw-bold':'item'"
              @click="handleTabClick('0')">使用说明</view>
        <view :class="tabIndex === '1' ? 'active item fw-bold':'item'"
              @click="handleTabClick('1')">注意事项</view>
      </view>
      <view class="section " v-if="tabIndex === '0'">
        <view class="title flex-h flex-c-s fw-bold">
          <image class="icon-title mr-16"
                 src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/icon-title.png"
                 mode="scaleToFill" />
          <view>收银方式</view>
        </view>
        <view class="pay-type flex-h flex-c-b">
          <view class="flex-h flex-c-c">
            <image class="pay-icon"
                   src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/person-pay.png"
                   mode="scaleToFill" />
            <view class="text">人工收银</view>
          </view>
          <view class="flex-h flex-c-c">
            <image class="pay-icon"
                   src="https://ggllstatic.hpgjzlinfo.com/static/supermarket/machine-pay.png"
                   mode="scaleToFill" />
...
export default {
  ...
  methods: {
    ...
    handleOtherMarket(){
      ...
      uni.navigateTo({
         url: '/pages/supermarket/other-market?info=' + encodeURIComponent(JSON.stringify(this.info))
      });
    },
    // 去付款事件
    handleAgreeClick(){
    ...
    async succFlag(flag) {
      if(flag == 1){
        ...
        uni.navigateTo({
          url: `/pages/user-center/real-name-result2?back=${'/pages/supermarket/market'}&info=${encodeURIComponent(JSON.stringify(this.info))}`
        })
