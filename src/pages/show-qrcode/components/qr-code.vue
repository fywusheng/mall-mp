<template>
  <view class="qr-code">
    <view class="card flex-v m-32 p-32 br-16">
      ...
      <text class="info fs-40 c-white mb-32">姓名：{{ info.userName | nameFilter }}</text>
      <text class="info fs-36 c-white mb-32">身份证：{{ info.idNo | idCardNumberFilter }}</text>
      <view v-if="info && info.userName" class="container flex-v flex-c-s bg-white br-16 p-32">
        <canvas canvas-id="bar-code" id="bar-code" class="bar-code" @click="showsBarCode = true" />
        <canvas
          canvas-id="qr-code"
          id="qr-code"
          class="qr-code mb-80"
      ...
      <view v-else class="container flex-v flex-c-s bg-white br-16 p-32">
        <image
          class="icon-code"
          src="http://192.168.1.187:10088/static/show-qrcode/icon-code-bg.png"
          mode="scaleToFill"
        />
        ...
        <view class="flex-h flex-c-c">
          <button class="button fs-40 ml-16 btn-get-code" hover-class="none" @click="handleReceive">
            立即领取
          </button>
        </view>
      </view>
