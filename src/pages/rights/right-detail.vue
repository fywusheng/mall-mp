...
<template>
  <view class="right-detail">
    ...
    <!-- #ifdef MP-ALIPAY -->
    <navigation-bar  title="优待说明"  title-color="#ffffff" :alpha="1"
                    background-color="linear-gradient(to right, #ff8800, #ff5000)"/>
    <!-- #endif -->
    <!-- #ifdef MP-WEIXIN -->
    <navigation-bar title="优待说明"
                    title-color="#ffffff"
                    background-color="linear-gradient(to right, #ff8800, #ff5000)"
...
export default {
  ...
  methods: {
    ...
    setContents(data) {
      ...
      this.contents = keys.map((item) => {
        if(!json[item]) return false
        //获取key，对key进行排序
        let arr = Object.keys(json[item]).sort((a, b) => {
    ...
    getBannerImageURL(index) {
      return `https://ggllstatic.hpgjzlinfo.com/static/rights/banner-rights-detail-${index}.png`
    },
  },
}
</script>
