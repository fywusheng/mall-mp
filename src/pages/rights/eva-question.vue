...
<template>
  <view class="evaluation-questions">
    <view class="top-tip">
      <view class="icon-tip">
        <image class="tip" src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-tip.png"
          mode="scaleToFill" />
      </view>

      <view class="txt">为保障结果真实有效，请家属采用一问一答的方式进行评估，由提问者进行填写并提交。</view>
    </view>
    <view class="subject">
      <view v-for="(subject, subIndex) in subject" class="subject-list" :key="subIndex">
        <view class="subject-title">
          <text class="red">*</text>
          <view class="cont">{{subject.title}}
        ...
        <view class="subject-cont">
          <radio-group @change="radioChange($event, subIndex)">
            <label class="uni-list-cell uni-list-cell-pd" v-for="item in subject.cont"
              :key="item.value">
              <view>
                <radio class="radio" color="#FF9500" :value="item.score"
  ...
  data() {
    return {
      subject: [
        ...
        {
          select: '1',
          title: '8.将100连续减7，共减5次，每减1个7，能否说出正确答案？（只能进行心算，期间不能进行明显的提示）?',
          cont: [
        ...
        {
          select: '1',
          title: '10.取出身边两个物品（如手表、铅笔、手机等）让老人辨认，老人是否能正确辨认并说出?',
          cont: [
        ...
        {
          select: '1',
          title: '12.做张纸条，上面写上“闭上你的眼睛”，老人是否能正确认读，并按照字面意思做出相应闭眼的动作?',
          cont: [
            { label: '是', value: '0', score: 1 },
            { label: '否', value: '1', score: 0 }
          ]
        ...
        },
        {
          ...
          imgs: 'https://ggllstatic.hpgjzlinfo.com/static/rights/icon-question.png',
          cont: [
            { label: '是', value: '0', score: 1 },
            { label: '否', value: '1', score: 0 }
          ]
      ...
      submitUrl: 'https://ggllstatic.hpgjzlinfo.com/static/rights/icon-submit.png'
    }
  },
  onLoad() {
