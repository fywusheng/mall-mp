<template>
  <view class="find">
    ...
    <view class="search-view flex-h flex-c-b bg-white p-0-32" :style="{ 'margin-top': navHeight + 'px' }">
      ...
      <button v-if="keyword" class="cancel-button fs-36 c-black ml-24" hover-class="none">取消</button>
    </view>
    <view class="content">
      <tab-list
        :list="list"
...
<script>
  ...
  import TabList from './find/components/tabs-swiper.vue';
  import YAudio from './find/components/audio.vue';
  ...
  export default {
    ...
    methods: {
      ...
      goVedio(e, tab, index) {
        ...
        uni.navigateTo({
          url: `/pages/find/video-list?firstVideo=${encodeURIComponent(JSON.stringify(firstVideo))}`,
        });
      },
      /**
       * 小视频详情
      ...
      handleNoimg(index, tab) {
        ...
        // #endif
        // #ifdef MP-ALIPAY
        this.innerAudioContext.src = this.list[currentIndex].content[index].youkuUrl || this.list[currentIndex].content[index].mediaUrl;
        // #endif
    ...
    onShow() {
      ...
      if (typeof curPages.getTabBar === 'function' && curPages.getTabBar()) {
        curPages.getTabBar().setData({
          tabIndex: 2, // 表示当前菜单的索引，该值在不同的页面表示不同
        });
      }
    },
    destroyed() {
