  ...
  <view class="article-detail">
    ...
    <view class="audio">
      <view class="audio-content flex-c-c">
        <view class="left flex-c-c">
          <cu-progress ref="cuProgress" :value="progress" @dragging="dragging"
            @dragstart="dragstart" @dragged="dragged" :current="current" :duration="duration"
            @dragcancel="dragcancel">
          </cu-progress>
        </view>
        <view class="right">
          <view class="flex-h flex-c-c article-content" @click.native.stop="handleNoimg()">
            <image v-if="!play" mode="scaleToFill"
              src="https://ggllstatic.hpgjzlinfo.com/static/find/horn2x.png"></image>
            <image v-if="play && !paused" class="playimg" mode="scaleToFill"
    ...
    <view v-if="isShare == 'true'" class="flex-c-c share-bot">
      <view class="share-item" @click="handleCollect">
        <image class="cole" mode="scaleToFill" v-if="detail.colFlag === '1'"
          src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-collected.png" />
        <image class="cole" mode="scaleToFill" v-else
...
export default {
  ...
  methods: {
      ...
      api.getContById({
        ...
        success: (res) => {
          ...
          if (res) {
            ...
            const s = handlerStr.replace(/<img/g, '<img style="width:100%;margin-left:-40px" mode="scaleToFill"')
            var regEx = /\s+/g
    ...
    handleImageLoadFail() {
      // 图片加载失败时显示默认图片
      this.detail.image = 'https://ggllstatic.hpgjzlinfo.com/static/home/image-home-article-default.png'
    },
    handleNoimg() {
      if (!this.play) {
        this.innerAudioContext.play()
  ...
  onShareTimeline() {
    return {
      ...
      imageUrl: this.imgs ? this.imgs[0] : 'https://ggllstatic.hpgjzlinfo.com/static/common/bg-share.png',
      success(res) {
        uni.showToast({
          title: '分享成功'
        })
