<template>
  <div>
    <hr-pull-load @refresh='refresh' @loadMore='loadMore' :height='-2' :pullHeight='50'
      :maxHeight='100' :lowerThreshold='30' :bottomTips='bottomTips' :isAllowPull="false"
      :isTab='true' ref='hrPullLoad'>
      <!-- 插入自己的数据-->
      ...
      <view class="content">
        ...
        <block v-for="(artItem,artIndex) in list" :key="artIndex">
          <!-- 听文章没图片模块 -->
          <view @click="goDetail($event,artIndex)" :data-id="artItem.contId"
            ...
            <rich-text class="noimg-title" :nodes="artItem.ttl | changeNode(keyword)"></rich-text>
          ...
          <view @click="goDetail($event,artIndex)" :data-id="artItem.contId"
            ...
            <view class="oneimg-title">
              ...
              <rich-text class="left-text" :nodes="artItem.ttl | changeNode(keyword)"></rich-text>
              <image :src="imgItem" v-for="(imgItem,imgIndex) in artItem.imgs" mode="scaleToFill"
                :key="imgIndex"></image>
            </view>
          ...
          <view @click="goDetail($event,artIndex)" :data-id="artItem.contId"
            ...
            <view class="img-center">
              <template v-for="(imgItem,imgIndex) in artItem.imgs">
                <image v-if="imgIndex<3" :src="imgItem" mode="scaleToFill"
          ...
          <!-- 小视频视频模块不上暂时屏蔽 -->
          <view @click="goVedio($event,artIndex)" :data-id="artItem.contId" class="content-noimg"
            v-if="false">
            <!-- <view class="audio-title">{{artItem.ttl}}</view> -->
            <view class="video-text">
              <rich-text class="audio-title" :nodes="artItem.ttl | changeVideo(keyword)">
              </rich-text>
            </view>
...
export default {
  ...
  methods: {
    changeColor(item, keyword) {
      ...
      item = item.replace(res, "<text style='color:red;'>" + keyword + '</text>')
      return item
    },
    // 点击文章详情
    goDetail(e, index) {
