<template>
  <view>
    <view
      class="cu-progress-main"
      :style="{
        'border-radius': bgBR,
        width: progressMainW,
        height: progressMainH,
        'background-color': backgroundColor,
      }"
    >
      <view class="flex-h flex-c-b show-time">
        <view class="mr-3">
          {{ getTime(Math.round(current)) }}
          <!-- 3:98 -->
        </view>
        <view class="ml-3">
          <!-- 3:98 -->
      ...
      <view
        class="cu-progress"
        :style="{ left: areaLeft, 'flex-direction': flexDirection, width: areaW, height: areaH }"
      >
        <view
          class="cu-progress-bar"
          :style="{
            bottom: pgBarBottom,
            background: pgBarBg,
            'margin-left': pgBarML,
            'margin-bottom': pgBarMB,
            width: pgBarW,
            height: pgBarH,
            'border-radius': pgBarBR,
            'background-color': noActiveColor,
          }"
        ></view>
        <movable-area
          class="cu-area"
          :style="{ 'flex-direction': flexDirection, width: areaW, height: areaH }"
          @touchcancel="touchCancel"
          @touchstart="areaTouchStart"
          @touchmove="areaTouchMove"
          @touchend="touchEnd"
        >
          <movable-view
            class="cu-handle"
          ...
          >
            <image
              class="icon"
              src="http://192.168.1.187:10088/static/find/currentTime.png"
              mode="scaleToFill"
            />
            <text
              :class="handleIcon"
              :style="{ backgroundColor: iconColor, 'font-size': iconS, 'border-radius': iconBR }"
            ></text>
          </movable-view>
        </movable-area>
      </view>
      ...
      <text
        v-if="getShowInfoStatus2view"
        class="cu-showInfo"
        :style="{ right: infoRt, left: infoLt, color: infoColor, 'font-size': infoS, width: infoW }"
      >
        {{ showValue }}{{ infoEndText }}
      </text>
    </view>
  </view>
</template>
...
<script>
// https://ext.dcloud.net.cn/plugin?id=1339   （所用插件）
export default {
  data() {
    return {
      scale: 1,
  ...
  beforeMount: function () {
    ...
    this.percent =
        (Math.abs(this.showValue - this.minValue) / Math.abs(this.maxValue - this.minValue)) * 100
    const minl = this.textLength(this.minValue + this.infoEndText)
    const maxl = this.textLength(this.maxValue + this.infoEndText)
  ...
  props: {
    ...
    activeColor: {
      default: '#333333', // 已经过区域颜色
      type: String
    },
    noActiveColor: {
      default: '#D2D2D2', // 为经过区域颜色
      type: String
    },
    strokeWidth: {
      default: '2', // 行程宽度
    ...
    width: {
      default: '420rpx', // 进度条总宽，如果显示百分比，则包括百分比在内的宽度
      type: [String, Number]
    }
  },
  computed: {
    ...
    showVLC() {
      const minl = this.textLength(this.minValue.toFixed(0) + this.infoEndText)
      const maxl = this.textLength(this.maxValue.toFixed(0) + this.infoEndText)

      const L = maxl > minl ? maxl : minl
      return L
    },
    flexDirection() {
      if (this.direction == 'vertical') {
    ...
    handleS() {
      const s = this.sizeDeal(this.handleH)
      return s[2]
    },
    handleL() {
      if (this.direction == 'vertical') {
    ...
    maxValue() {
      const max = Number.isNaN(parseFloat(this.max)) ? 100 : parseFloat(this.max)
      return this.valueFormat(max)
    },
    minValue() {
      const min = Number.isNaN(parseFloat(this.min)) ? 0 : parseFloat(this.min)
      const si = this.getStepInfo()
  ...
  watch: {
    ...
    value(v) {
      ...
      if (!this.handleMoveStatus) {
        ...
        this.percent =
            ((this.showValue - this.minValue) / Math.abs(this.maxValue - this.minValue)) * 100
        if (this.direction == 'vertical') {
          this.handleY = (this.progressBarInfo.height * this.percent) / 100
        } else {
          this.handleX = (this.progressBarInfo.width * this.percent) / 100
          console.log(this.handleX, this.handleY)
        }
      }
    }
  },
  methods: {
    ...
    valueFormat(v) {
      ...
      const valueE = v * 10 ** stepInfo[1]
      const remainder = valueE % (stepInfo[0] * 10 ** stepInfo[1])
      const remainderInt = Math.floor(remainder)
      const value = (Math.floor(valueE) - remainderInt) / 10 ** stepInfo[1]
    ...
    getStepInfo() {
      ...
      if (step <= 0 || !step) {
        ...
        const steps = step.toString().split('.')
        if (steps.length == 1) {
          return [step, 0]
        } else {
    ...
    clientInit() {
      this.$nextTick(function () {
        ...
        if (this.maxValue - this.minValue == 0) {
          ...
          const stepInfo = this.getStepInfo()
          const v = (this.percent * (this.maxValue - this.minValue)) / 100
          const valueE = v * 10 ** stepInfo[1]
          const remainder = valueE % (stepInfo[0] * 10 ** stepInfo[1])
          const remainderInt = Math.floor(remainder)
          const sv = (Math.floor(valueE) - remainderInt) / 10 ** stepInfo[1]
          this.showValue = sv
        }
      })
    },
    ...
    textLength(t) {
      t = t.toString()
      const int = t.split('.')[0]
      const subt = t.match(/[^\x00-\xff]/g)
      let subl = 0
      if (subt) {
        subl = subt.length
      }
      const l = ((int.length - subl + this.getStepInfo()[1]) / 3) * 2 + subl + 0.2
    ...
    maxHeight(vertical) {
      ...
      if (!vertical) {
        ...
        const subt = this.infoEndText.match(/[^\x00-\xff]/g)
        if (subt) {
      ...
      h.push(this.sizeDeal(this.handleH)[0])
      h.sort(function (a, b) {
        return b - a
      }) // 降序
    ...
    handleMove(x_) {
      ...
      if (this.direction == 'vertical') {
        ...
        cp = cp <= 1 ? cp : 1
        cp = cp > 0 ? cp : 0
        const value = Number((1 - cp) * (this.maxValue - this.minValue)) + this.minValue
        ...
        cp = cp <= 1 ? cp : 1
        cp = cp > 0 ? cp : 0
        const value = Number(cp * (this.maxValue - this.minValue)) + this.minValue
        this.percent = cp * 100
        this.showValue = this.valueFormat(value)
        this.handleX = x
      }
