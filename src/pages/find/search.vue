    ...
    <view class="histroy" v-if="!keyword && list.length === 0">
      ...
      <view class="histroy-serach" v-if="histroySearch.length > 0">
        ...
        <view class="histroy-serach-right" @click="clearHistory">
          <image src="http://192.168.1.187:10088/static/map/delete2x.png" mode="scaleToFill"></image>
          <text>清空</text>
        </view>
      </view>
      <view class="histroy-item" v-for="(item, index) in histroySearch" @click="setHistory(index)" :key="index">
        <text class="histroy-item-left">{{ item }}</text>
        <view class="histroy-item-right" @click.stop="deleteKey(index)">
          <image src="http://192.168.1.187:10088/static/find/delete2x.png" mode="scaleToFill"></image>
        </view>
      </view>
    </view>
    <!-- 搜索提示模块 -->
    <view class="hoot" v-if="!isResult">
      <view class="hoot-item" v-for="(item, index) in findList" :key="index" @click="handleHoot(item)">
        <view class="hoot-left">
          <image src="http://192.168.1.187:10088/static/map/icon-map-search-around.png" mode="scaleToFill"></image>
          <!-- <text>{{item}}</text> -->
        </view>
        <rich-text class="richtext" :nodes="item | changeNode(keyword)"></rich-text>
        <view class="hoot-right">
          <image src="http://192.168.1.187:10088/static/find/topLeft2x.png" mode="scaleToFill"></image>
        </view>
      </view>
    </view>
    <!-- 搜索结果显示模块 -->
    ...
    methods: {
      ...
      initRecordRecognitionManager() {
        ...
        manager.onRecognize = function (res) {
          console.log('语音输入结束onRecognize', res);
          self.content = res.result.replace('。', '').replace('.', '');
        };
        manager.onStop = function (res) {
          console.log('语音输入结束onStop', res);
          self.keyword = res.result.replace('。', '').replace('.', '');
        };
       ...
       */
      getHighline(str) {
        return str.replace(new RegExp(this.keyword, 'g'), `<text style="color: #0059FF;display:inline">${this.keyword}</text>`);
      },
      /**
       * 文章详情
       */
