  ...
  <view class="search">
    ...
    <view class="search-view flex-h flex-c-b bg-white p-0-32">
      <view class="search-bar flex-h flex-c-b flex-1 p-0-32">
        <input v-model="keyword" :focus="false" mode="scaleToFill" :maxlength="maxlength"
          class="search-bar__input fs-36 c-black flex-1" placeholder="搜你想看的" />
        ...
        <template>
          ...
          <image class="search-bar__voice ml-16"
    ...
    <view class="histroy" v-if="!keyword && list.length === 0">
      ...
      <view class="histroy-serach" v-if="histroySearch.length > 0">
        ...
        <view class="histroy-serach-right" @click="clearHistory">
          <image src="https://ggllstatic.hpgjzlinfo.com/static/map/delete2x.png" mode="scaleToFill">
          </image>
          <text>清空</text>
        </view>
      </view>
      <view class="histroy-item" v-for="(item,index) in histroySearch" @click="setHistory(index)"
        :key="index">
        <text class="histroy-item-left">{{item}}</text>
        <view class="histroy-item-right" @click.stop="deleteKey(index)">
          <image src="https://ggllstatic.hpgjzlinfo.com/static/find/delete2x.png"
    ...
    <view class="hoot" v-if="!isResult">
      <view class="hoot-item" v-for="(item,index) in findList" :key="index"
        @click="handleHoot(item)">
        <view class="hoot-left">
          <image src="https://ggllstatic.hpgjzlinfo.com/static/map/icon-map-search-around.png"
            mode="scaleToFill"></image>
          <!-- <text>{{item}}</text> -->
        </view>
        <rich-text class="richtext" :nodes="item | changeNode(keyword)"></rich-text>
        <view class="hoot-right">
          <image src="https://ggllstatic.hpgjzlinfo.com/static/find/topLeft2x.png"
            mode="scaleToFill"></image>
        </view>
      </view>
    </view>
    ...
    <view class="result" v-if="isResult">
      <result ref="result" :list="list" :isResult="isResult" :keyword="keyword" :stopPlay="stopPlay"
        @handleNoimg="handleNoimg" @handleSwitchPlay="handleSwitchPlay" @goDetail="goDetail"
        @goVedio="goVedio" @loadMore="loadMore"></result>
    </view>
    <!-- 音频组件控制模块 -->
    <view class="bg-black" v-if="!closeAudio" @click="closeAudio= true"></view>
    <view class="audio" v-if="showAudion">
      <Yaudio ref="yaudio" @handleClose="handleClose" @prePlay="prePlay"
        @handleSwitchPlay="handleSwitchPlay" @nextPlay="nextPlay" :title="title"
        :stopPlay="stopPlay" :closeAudio="closeAudio" @triggleBg="triggleBg"></Yaudio>
    </view>
    <!-- 语音弹出层模块 -->
  ...
  methods: {
    ...
    handleSearchClick() {
      ...
      if (a === undefined && this.keyword && this.histroySearch.length < 20) {
        this.histroySearch.unshift(this.keyword)
      } else if (a === undefined && this.keyword && this.histroySearch.length >= 20) {
