  ...
  <view class="article-detail">
      ...
      <view class="audio-content flex-c-c">
        ...
        <view class="right">
          <view class="flex-h flex-c-c article-content" @click.native.stop="handleNoimg()">
            <image v-if="!play" mode="scaleToFill" src="http://192.168.1.187:10088/static/find/horn2x.png"></image>
            <image v-if="play && !paused" mode="scaleToFill" src="http://192.168.1.187:10088/static/find/list-stop2x.png"></image>
            <image v-if="play && paused" mode="scaleToFill" src="http://192.168.1.187:10088/static/find/list-play2x.png"></image>
    ...
    <view class="flex-c-c share-bot">
      <view class="share-item" @click="handleCollect">
        <image class="cole" mode="scaleToFill" v-if="detail.colFlag === '1'" src="http://192.168.1.187:10088/static/map/icon-map-collected.png" />
  ...
  export default {
    ...
    onLoad(option) {
      ...
      if (option.playindex !== undefined) {
        ...
        if (this.list[this.playindex].imgs) {
          ...
          this.innerAudioContext.coverImgUrl = 'http://192.168.1.187:10088/static/find/process_bg2x.png';
        }
        console.log('到碗里来');
        // 判断当前是否在暂停
    ...
    methods: {
      ...
      handleListenArticleclick(index) {
        ...
        this.innerAudioContext.onTimeUpdate(() => {
          ...
          this.$refs.yaudio.process = (this.innerAudioContext.currentTime / this.innerAudioContext.duration) * 100;
          this.process = (this.innerAudioContext.currentTime / this.innerAudioContext.duration) * 100;
          if (this.index === index) {
            this.progress = (this.innerAudioContext.currentTime / this.innerAudioContext.duration) * 100;
            this.current = this.innerAudioContext.currentTime;
            this.duration = this.innerAudioContext.duration;
    ...
    onShareAppMessage(res) {
      return {
        ...
        imageUrl: this.imgs ? this.imgs[0] : 'http://192.168.1.187:10088/static/common/bg-share.png',
        success(res) {
          this.$uni.showToast({
            title: '分享成功',
          });
