  ...
  <view class="article-detail">
    ...
    <view class="line-audio">
      <view class="audio-content flex-c-c">
        ...
        <view class="right">
          <view class="flex-h flex-c-c article-content"
                @click.native.stop="handleNoimg()">
            <image v-if="!play" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/find/horn2x.png"></image>
            <image v-if="play && !paused" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/find/list-stop2x.png"></image>
            <image v-if="play && paused" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/find/list-play2x.png"></image>
            <text v-if="!play">听文章</text>
            <text class="play"
                  v-if="play && !paused">暂停</text>
            <text class="play"
                  v-if="play && paused">播放</text>
          </view>
        </view>
    ...
    <view class="flex-c-c share-bot">
      <view class="share-item"
            @click="handleCollect">
        <image class="cole" mode="scaleToFill"
      ...
      <view class="share-item"
        ...
        <image class="share" mode="scaleToFill"
    ...
    <uni-popup class="popup"
      ...
      <view class="share-pop pt-32">
        ...
        <view class="flex-h  list fs-36">
          <button class="item flex-v flex-c-c"
                  open-type="share">
          ...
          <button class="item flex-v flex-c-c" open-type="share">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-qq.png" mode="scaleToFill" />
            <text>QQ好友</text>
          </button>
          <button class="item flex-v flex-c-c" open-type="share">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-alipay.png" mode="scaleToFill" />
            <text>支付宝好友</text>
          </button>
          <button class="item flex-v flex-c-c" open-type="share">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-message.png" mode="scaleToFill" />
            <text>短信</text>
          </button>
          <!-- #endif -->
          <button class="item flex-v flex-c-c ml-56"
        ...
        <view class="btn">
          <view @click="handleCloseClick"
                class="cancle bg-white flex-h flex-c-c fs-44">取消</view>
        </view>
      </view>
    </uni-popup>
    <!-- 音频组件控制模块 -->
    <view class="bg-black"
          v-if="!closeAudio"
          @click="closeAudio= true"></view>
    <view class="audio"
          v-if="showAudion">
      <Yaudio @handleClose="handleClose"
              ref="yaudio"
              @prePlay="prePlay"
...
export default {
  components: { cuProgress, UniPopup, Yaudio },
  data() {
    return {
      // 文章id
      contId: '',
    ...
    if (option.playindex !== undefined) {
      ...
      if (this.list[this.playindex].imgs) {
        ...
        this.innerAudioContext.coverImgUrl =
          this.list[this.playindex].imgs[0]
      } else {
        this.videoBg = 'https://ggllstatic.hpgjzlinfo.com/static/find/process_bg2x.png'
        this.innerAudioContext.coverImgUrl = 'https://ggllstatic.hpgjzlinfo.com/static/find/process_bg2x.png'
      }
      console.log('到碗里来')
      //判断当前是否在暂停
  ...
  methods: {
    ...
    handleCopyClick() {
      uni.setClipboardData({
        data: 'https://ggll.hpgjzlinfo.com/#/discovery/app-detail/' + this.contId,
        success: (res) => {
