<template>
  <view class="city-picker">
    <!-- <navigation-bar :shows-back-button="true">
      <view class="p-8-0 bg-white">
        <view class="search-bar flex-h flex-c-s br-12">
          <image class="search-bar__icon ml-32" mode="scaleToFill"
            src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-search.png" />
          <input class="search-bar__input ml-16 fs-40 c-lightgrey flex-1" type="text"
            placeholder="城市" placeholder-class="placeholder" @input="handleInputChange" />
        </view>
      </view>
    </navigation-bar>
    <view class="blank" :style="{ height: navigationBarHeight + 'px' }" /> -->
    <scroll-view class="list" :style="{ height: scrollViewHeight + 'px' }" :scroll-y="true"
      :scroll-into-view="viewToScroll" @scroll="handleScrollViewScroll">
      <view class="section" v-if="!searchResult">
        <view class="section-header flex-h flex-c-b p-0-32">
          ...
          <button v-if="cities.recent.length > 0" class="fs-36 c-grey" hover-class="none"
            @click="handleClearClick">
            清除
          </button>
        </view>
        <view class="items flex-h flex-wrap ml-32 mr-72">
          <view v-if="cities.location" class="item flex-h flex-c-c mr-20 mb-20 p-0-8"
            @click="handleCityClick(cities.location)">
            <image class="item__icon mr-8" mode="scaleToFill"
          ...
          <view v-for="(item, index) in cities.recent"
            class="item flex-h flex-c-c mr-20 mb-20 p-0-16" :key="index"
      ...
      <view class="section" v-if="!searchResult">
        ...
        <view class="items flex-h flex-wrap ml-32 mr-72">
          <view v-for="(item, index) in cities.hot" class="item flex-h flex-c-c mr-20 mb-20"
            :key="index" @click="handleCityClick(item)">
            <text class="item__text fs-36 c-black">{{ item.name }}</text>
          </view>
        </view>
      </view>
      <view v-for="(section, sectionIndex) in list" class="section" :id="'section-' + section.index"
        :key="sectionIndex">
        <view class="section-header flex-h flex-c-s p-0-32" v-if="!searchResult">
          <text class="section-header__text">{{ section.index }}</text>
        </view>
        <view v-for="(city, cityIndex) in section.items" class="row flex-h flex-c-s pl-32 pr-32"
          :key="cityIndex" @click="handleCityClick(city)">
          <text class="row__text fs-36 c-black">{{ city.name }}</text>
        </view>
      </view>
    </scroll-view>
    <view class="index-list flex-v flex-c-c" v-if="!searchResult">
      <text v-for="(item, index) in indexes" class="item fs-32 animated"
        :class="item === selectedIndex ? 'c-primary' : 'c-grey'" :key="index"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      cities: {
        location: null,
        recent: uni.getStorageSync('citySearchHistories') || uni.getStorageSync('recentCities') || [],
        hot: [
