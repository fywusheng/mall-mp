  ...
  <view class="trade">
    ...
    <view class="center" v-if="list.length > 0">
      <view class="line" v-for="(item,i) in list" :key="i">
        <view class="parent" :class="{'clicked':selValue == item.id?true:false}"
          @click="selectJob(item)">
          <image v-if="selValue == item.id" class="left"
            src="https://ggllstatic.hpgjzlinfo.com/static/home/leftLight.png">
          </image>
          <view class="pname">{{item.postTypeName}}</view>
          <image class="right" v-if="selValue == item.id"
            src="https://ggllstatic.hpgjzlinfo.com/static/home/rightLight.png"></image>
          <image class="right" v-else src="https://ggllstatic.hpgjzlinfo.com/static/home/right.png">
          </image>
        </view>
        <view class="_right" v-if="selValue == item.id">
          <view class="child" :class="{'clickedChild':childValue == v.id}" @click="childClick(v)"
            v-for="(v,index) in item.childList" :key="index">
            <image v-if="childValue == v.id" class="left"
              src="https://ggllstatic.hpgjzlinfo.com/static/home/correct.png">
            </image>
            <view class="right_n" :class="{'rightClicked':childValue == v.id}">{{v.postTypeName}}
            </view>
          </view>
        </view>
      </view>
