...
<template>
  <view class="submit-result">
    ...
    <view class="flex-v flex-c-s">
      ...
      <text class="fs-40 c-grey mt-32" v-if="isForSelf">电子证照已提交申领审核</text>
      <text class="fs-40 c-primary" v-if="isForSelf">审核通过后您将获赠300积分</text>
      <text class="fs-40 c-grey mt-32" v-if="!isForSelf">预计将在2个工作日内完成审核</text>
    ...
    >
      <image
        class="credits-image"
        mode="scaleToFill"
        src="http://192.168.1.187:10088/static/family-account/image-common-credits-300.png"
      />
      <image
      ...
      <view slot="text">
        <view class="mainMin" v-if="reFale == 1">
          <view class="content">
            <text>
              您确定不添加赡养扶养关系吗？
              <text class="wen">这将会影响您在本平台享有的权益和服务</text>
            </text>
          </view>
        </view>
        <view class="main" v-if="isForSelf && reFale != 1">
          <view class="content">
            <text>
              根据
              <text class="wen">《中华人民共和国老年人权益保障法》</text>
              ,建议您添加"赡养抚养关系",否则将会影响您在本平台享有的权益和服务。
            </text>
          </view>
