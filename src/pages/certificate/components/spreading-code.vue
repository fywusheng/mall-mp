...
<template>
  <view class="qr-code">
    <view class="no-card bg-white br-24"
      ...
      <view class="btn-box">
        <button class="agree-button fs-44 fw-bold c-white"
                @click="handleGetButtonClick"> 立即领取 </button>
      </view>
    </view>

    <view class="card  flex-v mt-32 p-32 br-16"
          v-if="cardStatus !== 3 ">
      <image class="bg-code"
             src="https://ggllstatic.hpgjzlinfo.com/static/certificate/show-code-bg.png"
             mode="widthFix" />
      <view class="title flex-h flex-c-s mb-28 par_demo">
        <image class="img-logo mr-16"
               src="https://ggllstatic.hpgjzlinfo.com/static/certificate/img-logo.png"
               mode="scaleToFill" />
               <!-- TODO shili -->
        <image class="demo" src="https://ggllstatic.hpgjzlinfo.com/static/certificate/shili.png"  mode="scaleToFill"/>       
        <view class="fs-40 c-white">
          <view class="text fs-48 lh-72 fw-bold">中华人民共和国老年人证</view>
          <view class="text fs-36 made">中国老龄协会监制</view>
        </view>
      </view>
      <view class="info fs-36 c-white ">
        姓名：{{ userInfo.psnName | nameFilter }}
      </view>
      <view class="info fs-36 c-white ">
        身份证：{{ userInfo.idCard | idCardNumberFilter }}
      </view>

      <view class="container flex-v flex-c-s bg-white br-16 p-32"
            v-if="cardStatus == 1 && !showsBarCode && !showsQRCode">
        <view class="flex-h flex-c-c bar-code-text"
              @click="showNumber = !showNumber">{{ showNumber? info.ecQrCode:'点击可查看数字条形码'}}</view>
        <view v-show="closeModalS" class="flex-v flex-c-s">
           <canvas canvas-id="bar-code"
                id="bar-code"
                class="bar-code"
                @click="handleShowsBarCode" />
           <canvas canvas-id="qr-code"
                id="qr-code"
                class="qr-code"
        ...
        <view class="flex-h flex-c-c" v-if="!twoSecond">
          ...
          <button class="button fs-40 c-primary ml-16"
                  hover-class="none"
    ...
    <view class="preview-bar-code bg-white flex-h flex-c-c"
      ...
      <view class="before-bar-code flex-v flex-c-c"
            v-if="!sureSave">
        <image class="icon-save"
               src="https://ggllstatic.hpgjzlinfo.com/static/certificate/icon-save.png"
               mode="scaleToFill" />
        <view class="keep-warn">二维码用于身份认证和支付结算时向商家出示，请不要将二维码及数字发送给他人</view>
        <view class="btn-know flex-h flex-c-c"
              @click.native.stop="handleSave">知道了</view>
      </view>
      <view class="after-save flex-v flex-c-c"
            v-if="sureSave">
        <view class="codeNum">{{info.ecQrCode}}</view>

        <canvas canvas-id="bar-code-big"
                id="bar-code-big"
                class="image" />
    ...
    <view class="preview-qr-code bg-white flex-h flex-c-c"
          ...
          :style="{ display: true ? 'flex' : 'none' }"
          @click="closeQRCode">
      <canvas canvas-id="qr-code-big"
              id="qr-code-big"
              class="image" />
    </view>
    <view class="bottom_bg">
       <image class="img_bottom"
