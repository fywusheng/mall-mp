  ...
  <view class="qr-code">
    <view class="no-card bg-white br-24" v-if="cardStatus == 3">
      <view class="flex-h flex-c-s title">
        <image
          class="img-logo"
          src="http://192.168.1.187:10088/static/certificate/img-logo.png"
          mode="scaleToFill"
        />
      ...
      <view class="need-apply">
        <image
          class="img-apply"
          src="http://192.168.1.187:10088/static/certificate/img-apply.png"
          mode="scaleToFill"
        />
        <view class="text">
      ...
      <view class="btn-box">
        <button class="agree-button fs-44 fw-bold c-white" @click="handleGetButtonClick">
          立即领取
        </button>
      </view>
    </view>
    ...
    <view class="card flex-v mt-32 p-32 br-16" v-if="cardStatus !== 3">
      <image
        class="bg-code"
        src="http://192.168.1.187:10088/static/certificate/show-code-bg.png"
        mode="widthFix"
      />
      <view class="title flex-h flex-c-s mb-28 par_demo">
        ...
        <image
          class="demo"
          src="http://192.168.1.187:10088/static/certificate/shili.png"
          mode="scaleToFill"
        />
      ...
      <view class="info fs-36 c-white">姓名：{{ userInfo.psnName | nameFilter }}</view>
      <view class="info fs-36 c-white">身份证：{{ userInfo.idCard | idCardNumberFilter }}</view>

      <view
        class="container flex-v flex-c-s bg-white br-16 p-32"
        v-if="cardStatus == 1 && !showsBarCode && !showsQRCode"
      >
        <view class="flex-h flex-c-c bar-code-text" @click="showNumber = !showNumber">
          {{ showNumber ? info.ecQrCode : '点击可查看数字条形码' }}
        </view>
        <view v-show="closeModalS" class="flex-v flex-c-s">
          <canvas canvas-id="bar-code" id="bar-code" class="bar-code" @click="handleShowsBarCode" />
          <canvas canvas-id="qr-code" id="qr-code" class="qr-code" @click="handleShowsQRCode" />
        </view>
        ...
        <view class="flex-h flex-c-c" v-if="twoSecond">
          <button class="button fs-40 c-primary" hover-class="none">已刷新</button>
        </view>
      </view>
      <view class="container flex-v flex-c-c bg-white" v-if="cardStatus == 4 || cardStatus == 5">
        <image
          class="icon-tip"
          src="http://192.168.1.187:10088/static/certificate/icon-tip.png"
          mode="scaleToFill"
        />
    ...
    >
      <view class="before-bar-code flex-v flex-c-c" v-if="!sureSave">
        <image
          class="icon-save"
          src="http://192.168.1.187:10088/static/certificate/icon-save.png"
          mode="scaleToFill"
        />
        <view class="keep-warn">
          二维码用于身份认证和支付结算时向商家出示，请不要将二维码及数字发送给他人
        </view>
        <view class="btn-know flex-h flex-c-c" @click.native.stop="handleSave">知道了</view>
      </view>
    ...
    <view class="bottom_bg">
      <image
        class="img_bottom"
        src="http://192.168.1.187:10088/static/certificate/card_bottom.png"
        mode="scaleToFill"
      />
    </view>
