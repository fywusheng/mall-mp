<template>
  <view class="maintenance-and-support">
    ...
    <view class="form">
      <view class="item p-0-32" v-for="(item, index) in params.relations" :key="index">
        ...
        <view class="row flex-h flex-c-s p-20-0">
          ...
          >
            <text class="fs-40 c-black" :class="{ 'c-lightgrey': item.relation === '' }">
              {{ item.relation || '请选择' }}
            </text>
          </picker>
          <image
    ...
    <view class="save flex-h flex-c-c bg-white">
      <button class="button fs-44 c-white" @click="handleSaveButtonClick">保存</button>
    </view>
  </view>
</template>
