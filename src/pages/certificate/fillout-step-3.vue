  ...
  <view class="fillout-step-3">
    ...
    <view class="section">
      <view class="section-header flex-h flex-c-s p-0-32">
        <image class="section-header__icon" mode="scaleToFill"
      ...
      <view class="form">
        <view class="item p-0-32" v-for="(item, index) in params.materials" :key="index">
          <view class="row flex-h flex-c-s p-20-0">
            <!-- <text class="row__indicator">*</text> -->
            <text class="row__label--small fs-40 c-black">
              证明材料类型
            </text>
            <image class="row__illustration ml-12" mode="scaleToFill"
              src="https://ggllstatic.hpgjzlinfo.com/static/certificate/icon-certificate-illustration.png" />
            <picker class="flex-1" :id="index" :range="materialTypes"
              @change="handleMaterialTypeChange">
              <text class="fs-40 c-black flex-1 ml-36" :class="{ 'c-lightgrey': item.type === '' }">
                {{ item.type || "请选择材料类型" }}
              </text>
            </picker>
          ...
          <view class="row flex-h flex-c-b">
            ...
            <button class="see-example fs-36" hover-class="none"
              @click="handleSeeExampleClick(index)">
              查看示例
            </button>
          </view>
          <image v-if="item.url" class="photo mb-32" :src="item.url" mode="scaleToFill"
            @click="handlePhotoPickerClick(index)" />
          <view v-else class="photo-picker flex-v flex-c-c mb-32"
            @click="handlePhotoPickerClick(index)">
            <image class="photo-picker__icon" mode="scaleToFill"
              src="./static/icon-certificate-select-photo.png" />
            <text class="photo-picker__text fs-32 c-black mt-16">
              点击上传图片
            </text>
          </view>
          <button v-if="params.materials.length > 1" class="delete-button fs-40" hover-class="none"
    ...
    <button v-if="params.materials.length < materialTypes.length" class="add-button fs-40 c-primary"
      hover-class="none" @click="handleAddMaterialClick">
      添加更多证明材料
    </button>
    ...
    <view class="footer bg-white">
      ...
      <view class="actions flex-h flex-c-b mt-32">
        ...
        <button class="action fs-44 c-white bg-primary" @click="handleSubmitClick">
          提交审核
        </button>
      </view>
    </view>
