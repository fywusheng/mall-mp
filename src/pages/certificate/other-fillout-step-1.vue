  ...
  <view class="fillout-step-1">
    ...
    <view class="section">
      <view class="section-header flex-h flex-c-s p-0-32">
        <image class="section-header__icon" src="http://192.168.1.187:10088/static/certificate/icon-certificate-basic-info.png" />
      ...
      <view class="form m-0-32">
        <view class="row flex-h flex-c-s p-20-0" v-if="type === 'input'">
          ...
          <picker class="flex-1" :range="nations" @change="handleNationChange">
            <text class="fs-40 c-black flex-1 ml-48" :class="{ 'c-lightgrey': params.nation === '' }">
              {{ params.nation || '请选择民族' }}
            </text>
          </picker>
          <image class="row__accessory" src="http://192.168.1.187:10088/static/common/icon-common-arrow-rightward-grey.png" />
        </view>
        ...
        <view class="row flex-h flex-c-s p-20-0">
          ...
          <button @click="alipayChose" class="clickChose">
            {{ params.residentialCity || '请选择所在地区' }}
          </button>
          <!-- #endif -->
          <!-- #ifdef MP-WEIXIN -->
          <uni-data-picker
        ...
        <view class="row flex-h flex-c-s p-20-0" v-if="type === 'input'">
          ...
          >
            ...
            <text v-if="data.length == 0" class="city fs-40 c-black flex-1 ml-48" :class="{ 'c-lightgrey': data.length == 0 }">请选择</text>
          </uni-data-picker>
          <!-- #endif -->
          <image class="row__accessory" src="http://192.168.1.187:10088/static/common/icon-common-arrow-rightward-grey.png" />
        </view>
    ...
    <view class="footer bg-white">
      <button class="button fs-44 c-white" @click="handleNextStepClick">下一步</button>
    </view>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleResidentialCityChange(e) {
        this.params.residentialCity = e.detail.value.map((item) => item.text).join('');
      },
      /**
       * 办证机构改变回调
       */
      ...
      handleDomicileCityChange(e) {
        this.params.domicileCity = e.detail.value.map((item) => item.text).join('');
      },
      /**
       * 下一步点击事件
       */
