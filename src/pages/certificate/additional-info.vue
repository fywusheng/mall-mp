  ...
  <view class="additional-info">
    <view class="section">
      <view class="section-header flex-h flex-c-s p-0-32">
        <image class="section-header__icon" mode="scaleToFill" src="http://192.168.1.187:10088/static/certificate/icon-certificate-basic-info.png" />
      ...
      <view class="form m-0-32">
        <view class="row flex-h flex-c-s p-20-0">
          ...
          <picker class="flex-1 ml-48" :range="bloodTypes" @change="handleBloodTypeChange">
            <text class="fs-40 c-black" :class="{ 'c-lightgrey': params.bloodType === '' }">
        ...
        <view class="row flex-h flex-c-s p-20-0">
          ...
          >
            ...
            <text v-if="data.length == 0" class="city fs-40 c-black flex-1 ml-48" :class="{ 'c-lightgrey': data.length == 0 }">请选择所在地区</text>
          </uni-data-picker>
          <image class="row__accessory" mode="scaleToFill" src="http://192.168.1.187:10088/static/common/icon-common-arrow-rightward-grey.png" />
        </view>
        <view class="row flex-h flex-c-s p-20-0">
          <text class="row__indicator"></text>
          <input v-model="params.address" class="fs-40 c-black flex-1" placeholder="请输入详细地址" placeholder-class="placeholder" />
        </view>
        <view class="row flex-h flex-c-s p-20-0">
          <text class="row__indicator">*</text>
          ...
          <image
            ...
            mode="scaleToFill"
            src="http://192.168.1.187:10088/static/certificate/icon-certificate-illustration.png"
          />
          <picker class="flex-1 ml-36" :range="institutions" @change="handleInstitutionChange">
            <text class="fs-40 c-black" :class="{ 'c-lightgrey': params.institution === '' }">
    ...
    <view class="section">
      <view class="section-header flex-h flex-c-s p-0-32">
        <image
          ...
          mode="scaleToFill"
          src="http://192.168.1.187:10088/static/certificate/icon-certificate-contact-info.png"
        />
      ...
      <view class="form m-0-32">
        <view class="row flex-h flex-c-s p-20-0">
          ...
          <input class="fs-40 c-black flex-1" placeholder="请输入姓名" placeholder-class="placeholder" v-model="params.name" />
        </view>
        <view class="row flex-h flex-c-s p-20-0">
          <text class="row__indicator">*</text>
    ...
    <view v-if="needsMaterial" class="section">
      <view class="section-header flex-h flex-c-s p-0-32">
        <image
          class="section-header__icon"
          mode="scaleToFill"
          src="http://192.168.1.187:10088/static/certificate/icon-certificate-material-info.png"
        />
        ...
        <view class="item" v-for="(item, index) in params.materials" :key="index">
          <view class="row flex-h flex-c-s p-20-0">
            ...
            <image
              ...
              src="http://192.168.1.187:10088/static/certificate/icon-certificate-illustration.png"
            />
            <picker class="flex-1 ml-36" :id="index" :range="materialTypes" @change="handleMaterialTypeChange">
              <text class="fs-40 c-black" :class="{ 'c-lightgrey': item.type === '' }">
                {{ item.type || '请选择材料类型' }}
              </text>
            </picker>
            <image class="row__accessory" mode="scaleToFill" src="http://192.168.1.187:10088/static/common/icon-common-arrow-rightward-grey.png" />
          ...
          <view class="row flex-h flex-c-b">
            ...
            <button class="see-example fs-36" hover-class="none" @click="handleSeeExampleClick(index)">查看示例</button>
          </view>
          <image v-if="item.url" class="photo mb-32" :src="item.url" mode="scaleToFill" @click="handlePhotoPickerClick(index)" />
          <view v-else class="photo-picker flex-v flex-c-c" @click="handlePhotoPickerClick(index)">
            <image
              class="photo-picker__icon"
              mode="scaleToFill"
              src="http://192.168.1.187:10088/static/certificate/icon-certificate-select-photo.png"
            />
            <text class="photo-picker__text fs-32 c-black mt-16">点击上传图片</text>
          </view>
          <button v-if="params.materials.length > 1" class="delete-button fs-40" hover-class="none" @click="handleDeleteMaterialClick(index)">
            删除
          </button>
        </view>
        <button
    ...
    <view class="footer bg-white">
      <view class="tips">
        ...
        <text class="fs-36" style="color: #0097fe">以上填写信息为真实有效信息具有法律效益</text>
      </view>
      <view class="actions flex-h flex-c-b mt-32">
        <button class="action fs-44 c-primary bg-white border" @click="handlePreviousStepClick">上一步</button>
        <button class="action fs-44 c-white bg-primary" @click="handleSubmitClick">提交审核</button>
      </view>
    </view>
  </view>
</template>
