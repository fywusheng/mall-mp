  ...
  <view class="applicant-info">
    <view class="form m-32">
      <view class="row flex-h flex-c-s p-20-0">
        ...
        <input class="fs-40 c-black flex-1" placeholder="请输入姓名" :adjust-position="false"
          placeholder-class="placeholder" v-model="params.name" />
      </view>
      <view class="row flex-h flex-c-s p-20-0">
        <text class="row__indicator">*</text>
        <text class="row__label fs-40 c-black mr-48">身份证号(必填)</text>
        <input class="fs-40 c-black flex-1" type="idcard" :adjust-position="false"
          placeholder="请输入身份证号" placeholder-class="placeholder" v-model="params.idCardNumber"
      ...
      <view class="row flex-h flex-c-s p-20-0">
        ...
        <!-- #ifdef MP-ALIPAY -->
        <button @click="localeChose" class="clickChose">{{params.city || '请选择所在地区'}}</button>
        <!-- #endif -->
        <!-- #ifdef MP-WEIXIN -->
        <uni-data-picker class="flex-1" popup-title="请选择所在地区" :localdata="cities"
          @change="handleCityChange" v-slot:default="{data, error, options}">
          <!-- <view >
               <text
