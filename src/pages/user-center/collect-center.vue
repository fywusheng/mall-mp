...
<script>
  ...
  export default {
    components: { uniSegmentedControl, articleType, merchantType },
    data() {
      return {
        current: 1,
        items1: ['文章', '视频'],
    ...
    onShow() {
      ...
      if (this.current === 1) {
        const getArray = this.tablist[this.current].filter((item, index, array) => {
    ...
    onReachBottom() {
      // 上拉加载
      const changeData = { 0: this.businessList, 1: this.articalList, 2: this.articalList };
    ...
    methods: {
      ...
      fresh(mapVal) {
        ...
        const topflag = this.tablist[this.current][index]['topFlag'] == '1' ? '0' : '1';
      ...
      articalList() {
        ...
        api.findArticleCollectList({
          ...
          success: (res) => {
            const list = res.list || [];
            if (list.length > 0) {
              this.$set(this.tablist, this.current, this.tablist[this.current].concat(list));
              this.pageOption[this.current]['pageNum'] = this.pageOption[this.current]['pageNum'] + 1;
            }
