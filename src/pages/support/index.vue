<template>
  <view class="index supportName">
    ...
    <view class="tab-bar">
      <tab-bar :tabs="tabs" :initialIndex="selectedIndex" @change="handleTabChange" />
    </view>

    <view class="blank" style="height: 136rpx" />
    <view v-if="list.length > 0" class="list">
      <view v-for="(item, index) in list" class="item flex-v br-8 m-32" :key="index">
        <text class="name fs-44 fw-500 c-black">{{ item.supportName }}</text>
        <view class="info mt-36 pb-16">
          <view class="row flex-h">
            <text class="fs-36 c-lightgrey">{{ selectedIndex === 0 ? '赡养' : '扶养' }}关系：</text>
        ...
        <image
          class="stamp"
          mode="scaleToFill"
          src="http://192.168.1.187:10088/static/support/icon-support-stamp-1.png"
        />
      </view>
    </view>
    <view v-else class="no-data flex-v flex-c-s">
      ...
      />
      <text class="fs-36 c-grey mt-24">暂无{{ tabs[selectedIndex] }}信息</text>
      <image
        class="instruction"
        src="http://192.168.1.187:10088/static/support/image-support-instruction.png"
        mode="widthFix"
      />
    </view>
    ...
    >
      <image
        ...
        mode="scaleToFill"
        @click="detailSupport"
        src="http://192.168.1.187:10088/static/support/image-common-credits2-100.png"
      />
      <image
  ...
  export default {
    ...
    methods: {
      ...
      handleEditClick(index) {
        console.log(index);
        const list = this.selectedIndex === 0 ? this.list1[index] : this.list2[index];
        ...
        uni.navigateTo({
          ...
          success: (res) => {
            res.eventChannel.emit('didOpenPageFinish', {
              ...
              info: this.selectedIndex === 0 ? this.list1[index] : this.list2[index],
            });
          },
        });
      },
