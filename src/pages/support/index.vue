<template>
  <view class="index supportName">
    ...
    <view v-if="list.length > 0" class="list">
      <view v-for="(item, index) in list" class="item flex-v br-8 m-32" :key="index">
        ...
        <view class="actions flex-h flex-c-e">
          <button class="common-button fs-36 c-black ml-24" hover-class="none"
          ...
          <button v-if="item.isFamillyShip != '1'" class="bind-button fs-36 c-primary ml-24"
            hover-class="none" @click="handleBindClick(item)">
            绑定亲情账号
          </button>
        </view>
    ...
    <view class="credits-popup flex-v flex-c-c" v-if="showsCreditsPopup"
      ...
      <image class="credits-image" mode="scaleToFill" @click="detailSupport"
    ...
    <modal ref="popup" :cancelText='modal.cancelText' :confirmText='modal.confirmText'
