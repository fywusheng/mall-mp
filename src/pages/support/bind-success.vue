<template>
  <view class="add-success">
    ...
    <view class="card flex-v flex-c-c bg-white br-24">
      ...
      <text v-if="names.length > 0"
            class="message fs-36 c-lightgrey mt-16">
        已给
        <template v-for="(item, index) in names">
          <text class="c-primary"
                :key="index">【{{ item }}】</text>
          <text :key="'pause-' + index"
    ...
    <button class="button fs-44 fw-400 c-white"
            @click="handleButtonClick">
      {{ buttonText }}
    </button>

    <image class="img-bt" src="https://ggllstatic.hpgjzlinfo.com/static/support/image-support-rights-instruction.png" mode="scaleToFill" />
      ...
      if (this.names.length > 0) {
        ...
        params.success = () => {
          ...
          uni.getFileSystemManager().readFile({
            filePath: 'https://ggllstatic.hpgjzlinfo.com/static/support/05f08ce3a4c8ce77b5c0af61c35619c.jpg',
            encoding: 'base64',
            success: (res) => {
              api.clearBg({
