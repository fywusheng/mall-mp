<template>
  <view class="add">
    ...
    <view class="row flex-h flex-c-s m-0-32">
      ...
      <!-- #ifdef MP-WEIXIN -->
      <input class="flex-1 fs-40" placeholder="请输入姓名" placeholder-class="placeholder"
        v-model="params.supportName" :class="isEditable ? 'c-black' : 'color-lightgrey'"
        :disabled="!isEditable" :focus="true" @focus="currentFocus = 0" @blur="currentFocus = -1" />
      <image class="clear" mode="scaleToFill"
        src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-input-clear.png"
        v-if="params.supportName && currentFocus === 0" @click="params.supportName = ''" />
      <!-- #endif -->
      <!-- #ifdef MP-ALIPAY -->
      <input class="flex-1 fs-40" placeholder="请输入姓名" placeholder-class="placeholder"
        v-model="params.supportName" :class="isEditable ? 'c-black' : 'color-lightgrey'"
        :disabled="!isEditable" @focus="showIcon =  true" />
      <image class="clear" mode="scaleToFill"
        src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-input-clear.png"
        v-if="showIcon" @click="clearName" />
      <!-- #endif -->
    </view>
    <view class="row flex-h flex-c-s m-0-32">
      <text class="title fs-40 fw-400 c-black">身份证号</text>
      <input class="flex-1 fs-40" placeholder="请输入身份证号" placeholder-class="placeholder"
        type="idcard" maxlength="18" v-model="params.idCard"
        :class="isEditable ? 'c-black' : 'color-lightgrey'" :disabled="!isEditable"
        @focus="currentFocus = 1" @blur="currentFocus = -1" />
      <image class="clear" mode="scaleToFill"
        src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-input-clear.png"
        v-if="params.idCard && currentFocus === 1" @click="params.idCard = ''" />
    </view>
    <view class="row flex-h flex-c-s m-0-32">
      <text class="title fs-40 fw-400 c-black">联系电话</text>
      <input class="flex-1 fs-40 c-black" placeholder="请输入联系电话" placeholder-class="placeholder"
        type="number" maxlength="11" v-model="params.phone" @focus="currentFocus = 2"
        @blur="currentFocus = -1" />
      <image class="clear" mode="scaleToFill"
        src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-input-clear.png"
        v-if="params.phone && currentFocus === 2" @click="params.phone = ''" />
    </view>
    <view class="row flex-h flex-c-s m-0-32">
      <text class="title fs-40 fw-400 c-black">选择关系</text>
      <view class="types flex-h flex-c-b p-0-8">
        <text v-for="(item, index) in relations" class="type flex-1 fs-36 c-grey animated"
          :class="{ 'type--selected': index === selectedIndex }" :key="index"
          @click="selectedIndex = index">
          {{ item.title }}
        </text>
        <view class="highlight bg-white animated"
          :class="selectedIndex === 0 ? 'highlight--left' : 'highlight--right'" />
      </view>
    </view>
    <view class="relations flex-h flex-wrap m-32">
      <text class="item bg-white fs-36 c-black p-0-24 mr-48 mb-24 animated"
        v-for="(item, index) in relations[selectedIndex].items"
        :class="{ 'item--selected': index === selectedItemIndex }" :key="index"
        @click="selectedItemIndex = index">
        {{ item.relationName }}
      </text>
    </view>
    <view class="footer bg-white">
      <button class="add-button fs-44 fw-500 c-white m-0-32" @click="handleAddClick">
        确定
      </button>
    </view>
  </view>
...
export default {
  ...
  methods: {
     ...
     */
    setRelations() {
      console.log('33333333333333---:', this.supportId, 'index----', this.index)
