<template>
  <view class="add">
    ...
    <view class="footer bg-white">
      <button class="add-button fs-44 fw-500 c-white m-0-32" @click="handleAddClick">确定</button>
    </view>
  </view>
</template>
    ...
    handleAddClick() {
      if (this.isClick) {
        ...
        const data = {
          ...
          relationId: this.relations[this.selectedIndex].items[this.selectedItemIndex].relationId,
          supRelName:
              this.relations[this.selectedIndex].items[this.selectedItemIndex].relationName
        }
        api.addUserSupport({
          data,
          success: (res) => {
