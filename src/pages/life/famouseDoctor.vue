      ...
      <view class="doctor-list-item" v-for="item in doctorList" :key="item.id">
        <view class="top" @click="goDoctorPage(item.doctorUrl)">
          ...
          <view class="right">
            ...
            <view class="precent m-b">
              <view class="col">
                接诊率
                <text class="count">{{ item.acceptsRate }}%</text>
              </view>
              <view class="col">
                好评率
                <text class="count">{{ item.feedbackRate }}%</text>
              </view>
            </view>
            <view class="bk m-b t-2">擅长：{{ item.goodAt }}</view>
            <view class="label-wrapper m-b" v-if="item.adjustRecord">
              <view class="label" v-for="(el, ind) in item.adjustRecord.split(',')" :key="ind">
                {{ el }}
              </view>
            </view>
            <view>
        ...
        <view class="bottom" v-if="item.picPrice > 0 || item.phonePrice > 0">
          <view class="right" v-if="item.phonePrice > 0">
            <image class="icon" src="http://192.168.1.187:10088/static/seeDoctor/icon-phone.png" />
            <text>电话问诊费</text>
            <text class="price">¥{{ item.phonePrice }}</text>
          </view>
          <view class="left" v-if="item.picPrice > 0">
            <image class="icon" src="http://192.168.1.187:10088/static/seeDoctor/icon-img.png" />
            <text>图文问诊费</text>
