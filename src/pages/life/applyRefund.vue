 ...
 <view class="applyRefund">
    <!-- 订单详情 -->
     ...
     <view class="list">
       ...
       <view class="list-item">
         <view class="label">退款金额 <text class="grey">（实付金额）</text></view>
         <view class="cont">¥{{item.payAmount|formatMoney}}</view>
       </view>
       <view class="list-item">
       ...
       <view class="list-item">
         ...
         <view class="cont">
           <view class="return-list">
             <view v-for="(item, index) in reasons" :class="{item:true,active:index === selectIndex}" :key="item.id" @click="handleClick(index)">{{item.content}}</view>
           </view>
         </view>
       </view>
     </view>
     ...
     <view class="tip-cont">
        <view class="tip-logo-w">
          <image class="tip-logo" src="https://ggllstatic.hpgjzlinfo.com/static/life/warning-circle.png" mode="scaleToFill"/>
        </view>
        
        <view class="cont">申请退款一经提交 <text class="strong">不可撤销</text>,提交后将进行人工审核,审核通过后退款1-3个工作日内到账</view>
     </view>
