<template>
  <view class="hotel">
    <view v-if="list.length > 0">
      <view class="item" v-for="(item,index) in list" :key="index" @click="hostDetail(item)">
        <view class="top">
          <image :src="item.hotelDiscountLogo" class="bgimg" mode="scaleToFill" />
          <view class="top_c">
        ...
        <view class="bottom">
          <view class="infor">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/life/house.png" class="hourse" />
            <rich-text :nodes="item.show_num"></rich-text>
          </view>
          <view class="time">
            <image src="https://ggllstatic.hpgjzlinfo.com/static/life/time.png" class="hourse" />
...
export default {
  ...
  methods: {
    ...
    hostDetail(item) {
      uni.navigateTo({ url: '/pages/life/hotelDetail?hotelDiscountId=' + item.hotelDiscountId + '&type=2&isShowPrice=2&change=null' })
    },
    hotelList() {
      api.gethotelList({
