...
<view class="hotel_top">
    <view class="menuBar">
        <view @click="selectItem(1)">{{cityName}}<image class="triangle-icon ml-12" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-triangle-downward-black.png" /></view>
        <view @click="selectItem(2)">{{eatName}}<image class="triangle-icon ml-12" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-triangle-downward-black.png" /></view>
        <view @click="selectItem(3)">{{weekName}}<image class="triangle-icon ml-12" mode="scaleToFill"
                   src="https://ggllstatic.hpgjzlinfo.com/static/common/icon-common-triangle-downward-black.png" /></view>
    </view>
    <view class="hotel">
      <view v-if="list.length > 0">
          <view class="item" v-for="(item,index) in list" :key="index" @click.stop="goDetail(item)">
              <view class="top">
                  <view class="img_qy">
                  ...
                  <view class="top_c">
                        ...
                        <view class="price">
                          ...
                          <view class="right" @click.stop="goMap(item)">
                            <view class="li">|</view>
                            <view class="icon"><image class="im" src="https://ggllstatic.hpgjzlinfo.com/static/life/micon.png" mode="scaleToFill"/></view>
                            <view class="m">地图</view>
                          </view>
                        </view>
                  </view>
              </view>
              <view class="item_bottom" v-for="(items,index) in item.hotelDTOList" :key="index">
                  <view class="line">
  ...
  <uni-popup ref="notice" :maskClick="maskClick" type="bottom">
          <view class="panel">
            ...
            <view v-if="type == 1" class="content">
                  <view class="item" :class="{'colorItem':cityA == item.cityCode ? true : false}" @click="itemClick(item.cityCode,1,item.cityName)" v-for="(item,index) in showListValue.selectList" :key="index">{{item.cityName}}</view>
            </view>
            <view v-if="type == 2" class="content">
                  <view class="item" :class="{'colorItem':Bvalue == item.eatTime ? true : false}" @click="itemClick(item.eatTime,2,item.eatTime)" v-for="(item,index) in showListValue.selectList" :key="index">{{item.eatTime}}</view>
            </view>
            <view v-if="type == 3" class="content">
               <view v-if="false">{{demoType}}</view> 
               <view class="item" :class="{'colorItem':resultMap[item] == 1 ? true : false}" @click="itemClick(item,3,weekmap[item])" v-for="(item,index) in showListValue.selectList" :key="index">{{weekmap[item]}}</view>
 ...
 export default {
   ...
   data() {
    return{
       ...
       cityA:'',
       Bvalue:'全部餐段',
       weekmap:{'all':'全部星期','Mon':'星期一','Tues':'星期二','Wed':'星期三','Thur':'星期四','Fri':'星期五','Sat':'星期六','Sun':'星期日'}
     }
   },
   props:{
     hotelDiscountId:{
   ...
   methods:{
     goDetail(item){
      const params = {
       ...
       uni.redirectTo({url:"/pages/life/hotelHomeDetail?params="+`${encodeURIComponent(JSON.stringify(params))}`})
     },
     goMap(item){
      const params = {
        name: item.hotelName,
    ...
    cancel(){
     ...
     // this.resultMap = {}
      this.$refs.notice.close()
       ...
       switch(this.type){
        case 1: this.cityA = uni.getStorageSync('title');break;
         case 2: this.Bvalue = uni.getStorageSync('title');break;
         case 3: this.resultMap = uni.getStorageSync('title');break;
          return
      }
    },
    confirm(){ 
      ...
      const end_keys = keys.filter((key,index)=>{ return this.resultMap[key] == 1 })
      ...
      switch(this.type){
        case 1: this.cityName = this.name;break;
         case 2: this.eatName = this.name;break;
          case 3: this.weekName = week_name;break;
          return
      }
      this.dynamicQuery()       
      this.$refs.notice.close()
    ...
    selectItem(type){
     ...
     dynamicQuery(){
          ...
          if(keys.length > 0){
            const keyArray =  keys.filter((key,i)=>{ return this.resultMap[key] == 1} )
