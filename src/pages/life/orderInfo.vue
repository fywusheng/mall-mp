<template>
  <view class="orderInfo" v-if="info">
    ...
    <view class="top">
      <view class="top-left">
        ...
        <view v-if="info.orderStatus == 1">
          剩余
          <text class="red">{{ expirationTime[0] }}</text>
          分
          <text class="red">{{ expirationTime[1] }}</text>
          秒 订单自动取消
        </view>
        <view v-else-if="info.orderStatus == 2">
          还有
          <text class="red">{{ interval_time }}</text>
          过期,请尽快使用
        </view>
        ...
        <view class="panel-card" @click="goHotelDetail">
          ...
          <view class="card-cont">
            ...
            <view class="cont-right" @click.stop="goMap">
              ...
              <image
                class="icon-address"
                src="http://192.168.1.187:10088/static/life/micon.png"
                mode="scaleToFill"
              />
          ...
          <view v-if="info.productStatus == 2">
            ...
            <view v-if="isCancel == '2'">
              ...
              <view class="card-cont flex-left">
                <image
                  class="icon-tip"
                  src="http://192.168.1.187:10088/static/life/warning-circle.png"
                  mode="scaleToFill"
                />
      ...
      <view class="panel-body">
        <view class="static">
          ...
          <view class="txt" v-if="!['1', '4'].includes(info.orderStatus)">
            付款时间：{{ info.paymentTime | formatTime }}
          </view>
          <view class="txt">订单总价：¥{{ info.orderAmount | formaterMoney }}</view>
          <view class="txt" v-if="['1', '4'].includes(info.orderStatus)">
            应付金额：¥{{ info.payAmount | formaterMoney }}
          </view>
    ...
    <view v-if="info.orderStatus == 1" class="bottom_fix">
      <button class="btn active" @click="handlePay(info)">立即支付</button>
    </view>
    <view
      v-if="info.orderStatus == 2 && !(info.productStatus == 2 && isCancel == '2')"
