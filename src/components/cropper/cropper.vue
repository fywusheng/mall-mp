<template>
<view v-if="url" class="ksp-image-cutter">
	<canvas :style="{width: target.width + 'px', height: target.height + 'px',zIndex:'-1',position:'relative',top:'-100vh'}" canvas-id="target" id="target"></canvas>
	<view class="body">
		<image v-if="url" class="image"  mode="scaleToFill" @load="imageLoad" :style="{left: image.left + 'px', top: image.top + 'px', width: image.width + 'px', height: image.height + 'px'}" :src="url"></image>
...
<script>
// 插件地址：https://ext.dcloud.net.cn/plugin?id=1076  //用于裁剪图片
export default {
  props: {
    url: {
      type: String,
